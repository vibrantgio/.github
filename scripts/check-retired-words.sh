...
reach::*::ctx::(panic\(|unreachable\"\)|garbage|collect|retain|memory|import|package|graph|dead code|not reachable by|reachable from the root|lint|promotion|unreachable offset|struct|rather than from|chroma|registry|evict|handle|goroutine|unreachable here|unreachable code|device)::Dead-code idioms (panic("unreachable"), a vet-reported unreachable branch), memory and garbage-collector reachability, and package/import-graph reachability (a blank import, a promoted field, a struct leaf, a registry lookup, a raw handle staying valid, a stalled goroutine's eviction, a build-tag-gated function, a measurement device's connection) are ordinary English; only a control, row, item or command reachable or unreachable by keyboard, pointer or shortcut is the retired sense.

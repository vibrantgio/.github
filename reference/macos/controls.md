...
The push button's own corner is CG7.8.
...
| the push button's corner | **6 px** | same | a circular fit to the per-row coverage of the corner, the box's own extremes pinned — the fit CG5.3h made to the checkbox. Read off "Cancel", whose fill stands 19 of 255 off the sheet light and 16 dark, so a coverage read across that narrow a range is linear on whatever transfer the capture stores and needs none undone. Light: r = 6.11, rms 0.088 px over 8 rows. Dark: r = 6.17, rms 0.069 px over 8 rows. All four corners of the button agree to the hundredth in both appearances. The coverage missing from each corner says the same: 8.16 px² light and 7.94 px² dark against r²(1 − π/4), which is r = 6.17 and 6.08. Both sit above the radius drawn, for the reason every circular fit in this reference does — the platform's corner is a continuous curve, which departs from the straight edge further out than a circle and hugs it: the top row of this button leaves the fill at the seventh column in, where a circle of this fit would leave it at the sixth. "Save" read the same way gives r = 5.50 light and 5.51 dark and is NOT the reading carried: its fill spans 234 of 255 from the sheet, and across that range the capture's stored encoding is not a coverage. Read 2026-09-23 by CG7.8 |
...
| that control's edge UNDER THE POINTER, dark | `#4a5b65` against `#374852` at rest — `#485760` against `#374851` at the run's ends | `control-hover-dark.png`, `finder-window.png` | the tinted view pop-up's rim rows, y=8 and y=43, over the columns its corner does not round: at rest x 714-719 read `#374852` and x 712-713 and x 720-722 read `#374851`; hovered, x 716-720 read `#4a5b65` and x 712 and x 724 read `#485760`, the two rows agreeing at every column of that flat run. `hoverOverlay` — white at 0.094, the value the same capture's FILL gives — over the resting rim computes `#4a5962` against the measured `#4a5b65`, two of 255 short on green and three on blue, which is the tolerance this reference's other overlay readings carry. So the rim takes the pointer with the fill it stands in rather than staying put |
| the same, light | NONE | `control-hover-light.png`, `finder-window-light.png` | there is no rim to move: the hovered control's boundary rows at y=8 and y=43 read `#f3f3f3` against its own `#f2f2f2` fill and the `#fafafa` band above it, one 255th off the fill, which is the rounded rectangle's own antialiasing and not a stroke of a third value. The light appearance draws no rim at rest and none under the pointer |
...
replays it — `components/internal/controlface`'s `Cast`.
...
| the panel's width | 220 px, rim to rim | Screenshot 1 | the panel spans x 64–283, both columns its own rim, inside a window standing at x 56–1031 — 220 columns. Read 2026-09-18 with the inset above and spent by `patterns/sidebar` as `expandedDp` from CG5.8. No stored capture holds a COLLAPSED sidebar, so the rail's collapsed width has no reading and the pattern's own 48 stands |
...
**What the symbol's reading became.** The symbol reads 38 of 255 stronger
than the label standing beside it in light and 35 in dark, and no platform
name answers for black or white outright, so CG5.8 recorded the reading as a
third measured value of the rail beside the pill and the count:
`SidebarSymbol`, `#000000` light and `#ffffff` dark, `appkit:"-"`. On the
selected row's pill the symbol takes the pill's own foreground with the label
beside it.

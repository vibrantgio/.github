...
happens inside the sibling clones; the plan lives in `.github`, the
application guide in `workbench/llms.txt`. (Before G0E.1 the siblings hid under a gitignored `.repos/`
...
#### S22 — carried out of AN1.1's fresh-eyes review

Recorded by the orchestrator from the AN1.1 report (the worker
reported these without filing them). One of its "genuinely new" items
is struck here rather than recorded: "controls do not adopt the macOS
accent" is the frozen-scheme misread this file already carries — the
apps consume the OS accent through the live theme stream; a headless
frame shows one emission.

- The todos FAB sits on the window's bottom-right resize grip, a
  system-owned hit region — a sharper sub-point under the held FAB
  item (S13).
- todos' list rows carry no hairline separators, so the list has no
  structure to scan — app polish beside the S13 density item.
- Two checkbox drawings live in the org on different grids:
  `markdown/document.go` draws its own task checkbox at 14 dp with a
  2 px stroke (band/size 0.143) and its own check proportions;
  `components/input` now draws 20 dp on the icon set's 24-grid with a
  1.67 dp diagonal band (0.083). Both correct in isolation, visibly
  different weights side by side — vaultview shows one, todos the
  other. Which grid wins is a ruling; recorded, not taken.
- The published sheet's `--color-focus-ring` still points at
...
- [x] The section's heading bar and its grid agree on width at
...
- [x] Exit: renders at the truncating width and a wide one, both
...
- [x] Ruled by the owner on 2026-08-21: a pinned base that claims
...
- [x] Exit: renders in both schemes with a seed whose light pin
...
- [x] The outline pane's edges stop disagreeing: the pill, the
...
- [x] Three small disagreements settle: the heading-level indent
...
- [x] Focus takes one idiom across the library: the ring the text
  ...
  walkthrough saw no ring at all; the code strokes a grey ring
...
- [x] The launcher's cards fill at level 1 over the level-0 page,
...
## Phase AP: The published sheet draws what the component draws

AN1.1 gave the Gio checkbox a check glyph and a ramp-derived border;
the published CSS sheet (`design/styles.css`, generated by
`theme/export`) still ships the old form. The live mirror
(`design/mirror/TestComponentMirrors`) still passes but its margin
collapsed — `checkbox-checked.html` at 0.0159 against a 0.0170
tolerance — so the next perturbation on either side goes red across a
repo seam. The sheet also predates the focus-ring walk (ADR-014 S22).

### G-AP1: The sheet's checkbox matches the component's

#### AP1.1: The sheet draws the check and the derived border

In `theme/export` (`css.go`'s checkbox block ~L778–807 and the
"neutral 500 strong border" comment ~L485, `readme.go` ~L136 which
states the Gio side draws no checkmark): emit the check glyph and the
MarkOn-derived border rung per scheme, matching what
`components/input/checkbox.go` now draws. Judgment the task owns: the
class layer's no-`#` guard (`roundtrip_test.go` ~L415) fails any
data-URI glyph — reshape the guard deliberately or find an
encoding-free route (CSS clip-path/borders), never evade it. Bring
`--color-focus-ring` up to `focus.Ring`'s walk in the same pass.
Regenerate `design/styles.css` and `design/readme.md`; correct the
now-false comments in `design/mirror/fixtures/checkbox.html` and
...
AN1.1's derivation cleared the checkbox's border over level 0 and
exposed the general failure: the component knows no ground, and light
neutral 600 measures 2.94:1 over level 2 and 2.15:1 over level 3.
Eleven more sites hard-code `Ramps.Neutral.Step(500)` for the same
job — `components/input/radio.go` ~L177, `dropdown.go` ~L346/351,
`textfield.go` ~L557, `patterns/card`, `popover`, `modal`, `hero`,
`pricing`, `testimonial`, and two workbench apps — the same
scheme-blind pairing the checkbox just left.

### G-AQ1: The component row's borders clear the floor wherever they stand

#### AQ1.1: The checkbox derives its border against a Ground it is handed

Give `CheckboxProps`/`CheckboxRenderState` a `Ground
tokens.ElevationLevel` the way `button.RenderState.Ground` works
(zero value the window ground, so call sites keep their look), derive
the border with the MarkOn walk against that ground, and have the
...
#### AQ1.2: The remaining hard-coded neutral-500 borders re-derive

Sweep the eleven recorded sites (task body above; re-locate through
the graph — lines drift): each border or outline that pairs against a
ground re-derives via the tokens' contrast machinery against its
actual ground, taking the Ground-prop pattern where the ground varies
and a direct derivation where it is fixed. One coherent recipe, no

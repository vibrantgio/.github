...
431. **[bug]** **Other apps still fill a field on a chrome rail with the content's white.** CG1.1 made a text field's interior the surface it stands on — measured off the Save dialog's dark field, whose interior is the sheet's `#232a2f` and not the platform's `#1e1e1e` text background — and stated that surface in vaultview's rail alone. `workbench/feeds`, `workbench/iconbrowser`, `workbench/mindchat` and `workbench/sk150` each build a field without stating one; each needs reading for whether it stands on a rail.
...
439. **[decide]** **What a search field standing on a chrome rail is.** The reference answers three ways and CG1.1 painted the one the task named. The Save dialog's TEXT field fills with the sheet's own fill inside a `FieldEdge` hairline (`save-dialog-{light,dark}.png`), which on a `#f7f7f7` rail is a `#f3f3f3` stroke four levels off the surface — a fresh reader called it "a magnifier and some placeholder text with a ghost of a rectangle", plainly visible in dark and effectively absent in light. Voice Memos' SEARCH field is a flat `#e8e8e8` fill on a `#ffffff` toolbar, 36 px tall, with no stroke at all (`voicememos-sidebar-light.png`, col 900, y 46–81) — a recess carried by fill, and `#e8e8e8` is black at 23/255, which is no name in `nscolors.tsv`. Finder's untinted toolbar controls are LIGHTER than the band they stand on in both appearances — `#ffffff` on `#f7f7f7` light and `#262626` on `#1e1e1e` dark (`finder-window-untinted-{light,dark}.png`, row 60). One ruling: whether a search field is a text field, and whether a field's box is a recess, a lift or its own surface. A capture of a search field standing on a SIDEBAR would settle the case this window actually has.
...
446. **[decide]** **A scrolled column is the wrong shape for thirty-six names in a choices window.** CG2.1 gave the themer's syntax base group the box height the column of names could have, and it comes to five rows of thirty-six: a keyhole onto a list nobody can scan, which a fresh reader called "a complete list of five that happens to have a scrollbar next to it". `components/picker`'s Field is the platform's own answer — a pop-up button whose menu fits the available room and scrolls inside it — and adopting it would free the whole group to one row. What stops it today is seeding: the live path takes `Selected` on subscribe and does not re-read it, and the themer's applied base arrives with the model, so the field would open on whatever sorted first. A `Selected` the field re-reads, or the `components/keyed` idiom applied here, closes it.

447. **[decide]** **The themer's fenced sample is two lines where it was eight.** The sample now has to stand inside the syntax base group's box beside the column of names, and eight lines did not fit a window that also holds four other groups and a preview. Two lines still carry a comment, a keyword run, a string and a number, which is what the sample was argued down to; a palette's treatment of a nested block, a type declaration or a long string is no longer on screen. A sheet the group's title row opens, holding the full sample at a readable height, is the shape that would give all of it back. Joins 446.
...
481. **[decide]** **The themer's colour field does not follow the appearance switch.** The hex field is `components/input`'s live TextField, which reads its colours off the theme stream — the desktop's appearance — while every other thing in the window now draws in the appearance the switch is on. Switching to the side the desktop is not on therefore leaves one white field in a dark window. Handing the component a theme whose colour set is the switched one is three lines; what stops it is that `theme.Theme` carries its colours as a cold constant and the live stream emits a new theme per appearance poll, so a component that flattens the theme resubscribes the model — which is published without replay, and a resubscriber would be left with no colours until the next message. Replay-latest multicast (`rx.Behavior`) is the shape that settles it, for this window and for any other that draws a published component in an appearance of its own.

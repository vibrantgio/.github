...
## CO. From CG5.10, what one measured band costs the sizes under 24 dp

Filed 2026-09-18 from the work itself and from the fresh-eyes review of the
gallery's whole icon row in both schemes at 16, 20 and 24 dp, beside Finder's
own light toolbar band and Voice Memos' sidebar toggle. The review read this
task's own result back off the images without being given a number — the band
"dead centre" of the platform's 1.354 to 1.54, the magnifier's ring centreline
at 11.49 against the platform's 11.44 and its handle's tip at 13.3 against
13.33, the second band at 0.89 against 0.93 — and every finding it filed is
about what happens below 24 dp.

677. **[decide]** **The set's band is a fraction of the mark's size where the platform's is very nearly a fixed width.** `components/icons` draws 1.4 units of a 24-unit grid, so the band is 1.40 px at 24 dp, 1.17 at 20 and 0.93 at 16 — dp/17.07. MEASURED by the fresh eyes across four platform glyphs whose footprints span 5 px to 19 px: Finder's list-pop-up bars 1.354, Voice Memos' sidebar box 1.391, Finder's magnifier ring 1.475 and its pop-up chevron 1.54 slope-corrected — a spread of 14 per cent over a fourfold change of size. So at 16 dp this set lays 34 per cent less coverage per unit of band length than anything the platform draws at 1x, and the darkest pixel inside one row of the sheet spans 3.4× (the diagonal marks bottom out at sRGB 132, 114, 106 and 101 on white where the axis-aligned ones reach 39). Whether a mark's band should be held near 1.4 px at every size the library draws it at, rather than 1.4 units of its own grid, is unruled; it is the one change that closes this and the chevron pair's waist together.
678. **[decide]** **The gallery's icon sheet shows a control's mark at sizes the library never draws it at.** The chevron pair and the single chevron are a control's marks: `components/internal/control`'s `DrawMark` is the only caller and it draws into a 24 dp box. The sheet shows every mark at 16, 20 and 24 because it is one sheet, and the pair's measured clear row — five rows, one clear, five — survives at 24 dp alone (its waist carries 0.00 of a row there, 0.47 at 20 and 0.79 at 16, so the mark closes into a diamond), which is what a reviewer reads as a defect in a drawing nothing draws. Whether the sheet should show each mark at the sizes it is actually spent at is unruled.
679. **[decide]** **The sidebar folder's flap cannot separate from its top edge at any size, and its capture is why.** MEASURED, `voicememos-multi-folder-2026-09-18.png`: the flap stands 1.24 px clear of the body's inner top face over a mean band of 1.42, which is 0.87 of a band and 1.22 units against the set's 1.4 — under two device pixels at 24 dp, so no row between the two lines can be clear. The fresh eyes read the column profile as 1.00 / 0.39 / 0.40 / 1.00 at 24 dp and one fuzzy band of three to four rows below that, and could not tell `folder` from `open-folder` at 16 dp, where the two differ by 14.7 per cent of the folder's coverage against 27.8 at 24. `open-folder`'s own capture (`mail-window.png`) gives it 2.09 units of air and it holds a true zero at all three sizes. Widening the closed folder's gap to the open one's replaces a measurement with a copy; whether the platform's own 1.24 px is reproducible at all below 24 dp, or the two folders need different silhouettes rather than different gaps, is unruled.
680. **[decide]** **The diagonal allowance is the one number left in the icon grid that nothing measured.** The square keyline is 19 (CG5.7, Finder's tag) and the round and curved allowance is 13 (CG5.10, the magnifier's lens); a form whose every band runs on the diagonal still takes 20 units, and only `check` and `clear` take it. The fresh eyes measured the consequence: `clear` and `plus` are drawn to boxes of the same 18.60 px at 24 dp, and because the X's arms run corner to corner it carries 34 per cent more coverage than the plus at every size. No stored capture holds a bare diagonal band standing alone in a toolbar band — Notes' compose at 17 × 17 is a figure, not a band. One capture closes it: a toolbar control whose whole symbol is a diagonal cross or tick, window-bounded at 1x, in either appearance.
681. **[bug]** **The refresh mark's arrowhead clears its ring only at 24 dp.** MEASURED by the fresh eyes off the sheet: the figure's own bounding box is 10 × 10 px at 16 dp, 12 × 12 at 20 and 14 × 16 at 24, so it is the one mark in the set whose aspect moves with its size; at 16 dp the head is a 6 × 2 smear and the mark reads as a broken ring with a smudge rather than as an arrow. The head is 2.1 units either side of the band's own end face and 3.5 along the tangent, both derived from the band rather than read off anything. 425 already asks for Safari's reload symbol; this is what that capture has to settle.
682. **[bug]** **`workbench/vaultview/frame.go` is committed unformatted.** `gofmt -l` reports it: a stray blank line after `bandTrailingDp` at line 174. Nothing in CG5.10 touched the file, so it predates this round.

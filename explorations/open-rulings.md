...
673. **[note]** **A deferred drawing cannot be clipped by a box handed down the context.** `op.Defer` restores the transform and resets the clip, so the only clip a deferred drawing can carry is one recorded inside its own macro — written in the control's own coordinate space. Nothing in Gio 0.10.2 tells a control where that space sits: `layout.Context` carries constraints and no position, `layout.List` decides where a row goes only after every row has been laid out, and `op.Offset` is write-only. A viewport published by a scrolling container therefore cuts at the control's own origin, not the container's, and the error is the whole offset of the control within it. Measured: the components tab's focused push button lost the outer 2 dp of its band at x 286-292 where nothing was near an edge. CG5.9 landed the other mechanism instead — the band goes to a sink the control publishes just outside the one clip it must escape — which needs no coordinates and keeps every other clip.
674. **[note]** **A focused control's band now reads under its own drop shadow in a toolbar band.** A bordered control's shadow is deferred on purpose, so the band's one pass lands after every column; the focus band is no longer deferred, so the shadow's ramp is composited over the half of the band that lies outside the control's box. MEASURED off the regenerated goldens: at most 11 of 255 on `window-find-light.png`'s find recess and 3 of 255 on the search field's, over a ring the platform draws on top. What would close it is the shadow cutting the band's own footprint out of itself as it already cuts the control's box, which needs the shadow to know the control is focused.
675. **[bug]** **Two of the gallery's group goldens move on any change to a component's op stream.** Regenerating after CG5.9 moved 453 scattered pixels in `components-light.png` and 691 in `components-dark.png`, none by more than four of 255 and almost all by one, in text families the task does not touch. It is pool 568's finding — the glyph atlas packs differently when the op stream changes — now confirmed a second time from an unrelated change, and it means a real one-255th text regression stays invisible inside the churn.

...
## CT. From CG5.19, the push button's absent edge

Filed 2026-09-22 from the work itself. No fresh eyes were named; the boundary
rows were read back off the regenerated goldens against the captures and the
numbers are in the commit bodies.

721. **[note]** **The toolbar control's rim is recorded at rest only; the hover captures measure it too.** `controls.md`'s "The rim is the measured pixel" carries `toolbarControlRim` from the untinted Finder captures at rest. Reading the same control under the pointer: `control-hover-dark.png` gives `#485760` around the hovered view pop-up (x 694–741, y 8–43) where `finder-window.png` gives `#374851` around it at rest, both against a `#23292d` band; light draws none in either state, agreeing with the rest reading. So the rim lifts with the fill under the pointer rather than staying put, which nothing in the reference states and no consumer draws. One row in `controls.md` closes it.

722. **[note]** **The control the Ghost's hover overlay is read off is a bordered toolbar control, not a bare one.** `components/button`'s `buttonColors` cites the Finder toolbar pop-up "which carries no fill of its own at rest" as the case a Ghost button matches. At rest in dark that control does carry a rim (`#374851`, the reading above) and a fill one level off the band (`#242d32` against `#23292d`), which is what a BORDERED toolbar control draws — the chrome variant, not a Ghost. The overlay values themselves are unaffected, since the overlay lands on a fill indistinguishable from the band; what is unsupported is the claim that a bare control is what was measured. No stored capture holds a genuinely bare control — a toolbar's bare marks, a title, a link — under the pointer at all. One capture closes it: **a frontmost window whose toolbar holds a bare mark under the pointer**, window-bounded at 1x, both appearances.

...
436. **[decide]** **A window's two vertical boundaries read at different strengths.** The floating sidebar's edge measures #919191 light because the platform's separator lies on the window's plane there; the flush inspector's measures #e6e6e6 because it lies on the content. Both are the Seam entry applied correctly and the two do not look like one window. Whether a flush chrome boundary takes a second value, or the rule stands and the difference is the platform's, wants a ruling.

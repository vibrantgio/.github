...
## CI. From CG5.3g, the toolbar band as one across the window's columns

Filed 2026-09-18 from the measurement itself. No fresh eyes were named for
this task; the band's rows and the recess's rim were read back off the
regenerated goldens by the worker and are in the commit body.

588. **[bug]** **The stored captures draw the sidebar as an inset rounded box with its own rim, not a flush column.** MEASURED, `finder-window-untinted-dark.png` at 1x: the window's opaque bounds are x 56-1399 and its top row is y=38, but the sidebar's edge is a 1 px highlight at x=373 reading #3f3f3f at y=100 and #434343 at y=500, joined by a horizontal highlight of #3b3b3b along y=46 running the sidebar's whole width and by a second at x=64 down its leading side, the three meeting through a rounded corner at about (366, 52). Eight pixels of the window's own chrome stand outside each of those three edges. `finder-window-light.png` agrees in kind: the boundary pixel at x=317 reads #ffffff, LIGHTER than the #f9f9f9 sidebar beside it, and the content darkens to #f3f3f3 at x=318 recovering over 24 px, which is a rim and a cast shadow, not a seam. `patterns/pane`'s package comment records this same edge as a flush column parted by one line; pool 437 asks what the vertical boundary is painted with and this asks the question under it — whether macOS 26 still has a flush sidebar to model at all, or whether the pattern is an inset box on the window's plane with a rim and a shadow. One ruling settles both.
589. **[bug]** **In the light appearance no line stands at that boundary anywhere, band or below.** MEASURED, `finder-window-light.png` x 313-322 at y=80, 200 and 400 as well as at y=30, 40, 44, 48 and 51: no row carries a pixel darker than the two beside it. This task stopped the seam inside the band and left it below, which is what the ruling asked; below the band the light scheme still draws a #dfdfdf line the platform does not draw. It is pool 437's to settle, recorded here because CG5.3g measured it again on a second capture and the reading held.
590. **[decide]** **The band is one across two of vaultview's three columns and not the third.** The ruling names the sidebar's column, so `patterns/pane` stops its seam at the band and the rail splitter drawn over that seam stops with it. vaultview's trailing boundary — the note against the backlinks aside — is a splitter of its own on a content-side edge and still draws its line through the band, as does the aside's own full-height fill. Either the band's rule is the window's and every column's boundary stops at it, or it is the chrome column's alone and the trailing one is right as it stands.

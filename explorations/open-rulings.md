...
567. **[decide]** **A switched-off control's own mark reads under the graphic floor.** The switched-off check and the switched-off radio's dot take the colour the switched-off label takes, the platform's tertiary label over the faded fill, and read |Lc| 33 light and 17 dark against that fill where `tokens.GraphicFloor` is 45. It is the same order as the switched-off label's own 35.6 and −16.1 on the sheet, so it is the platform's relation and not a derivation — but nothing measured a switched-off control that still carries a mark, so the choice stands on the label alone. The capture is on 425's list; rule whether the mark follows the label there or keeps a floor of its own.

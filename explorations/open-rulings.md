...
## CN. From CG5.6, the bundle brought level with the library

Filed 2026-09-18 from the redraw itself. The mirror is the review for this
round: every pair is green with `Tolerance` and `pushButtonLabelFloor`
unchanged and the nearest wrong variant still failing, and the numbers are in
the `design` commit's body.

658. **[decide]** **The push button label floor now sits above both pairs it was measured for.** `pushButtonLabelFloor` 0.0265 was fitted when button-tonal read 0.0218 and dropdown 0.0251 against the wrong-radius variant's 0.0279. With the sheet no longer spending the roles' tracking they read 0.0178 and 0.0176, both under `Tolerance` 0.0223 — so half that floor was the sheet setting the label a fraction wider than the component, not the two rasterisers disagreeing. The ceiling was left standing rather than retired in the round that moved the drawings. Whether it retires now, and the two pairs are scored against `Tolerance` like everything else, is the decision.
659. **[bug]** **Nothing checks that a hand-authored page's `var()` references resolve.** All six of `design/components/*.html` styled their own chrome through eight `--color-*` variables that left the sheet with Phase CE and resolved to nothing for every round since: the page's background, its text colour, its specimen rows' fill and hairline and its annotation colour were all unset. `theme/export`'s `TestPageVarClosure` closes exactly the three GENERATED foundation pages; the hand-authored pages are in `design`, which has no such test, and `design/mirror` scores fixtures rather than pages. A closure test over `design.Bundle`'s own HTML against the emitted sheet would have caught it the day it broke.
660. **[decide]** **One measured shadow is now stated in two modules.** The bordered toolbar control's drop shadow is fitted once in `components/internal/control`, and `theme/export` restates its reach and offset because the module graph runs the other way — components imports theme, so the sheet cannot read the fit. `control.FloatingShadowReach` already carries the same duplication for the floating surfaces' 24. Whether measured geometry that both sides need belongs in `theme/tokens` beside the colour it is spent with, or stays stated twice with the provenance on each, is unruled.
661. **[note]** **A labelled toolbar control's side inset is unmeasured.** `control.ChromeMarkSideDp` 7 is read off four toolbar controls carrying ONE SYMBOL and nothing else. No stored capture holds a bordered toolbar control with a word in it, so `.btn.chrome` spends the S3 stop beside its label and `components/button`'s chrome variant is only ever drawn around a mark. The capture that would settle it belongs on 425's list.

...
625. **[decide]** **The sidebar's counts have no colour a capture can separate from text rendering.** In Screenshot 1 the section label "My Folders" reads `#7d7d7d`, which is `secondaryLabelColor` over the panel's fill to within half of 255, and the counts beside the rows read `#6d6d6d` on the same fill — 16 of 255 darker. Either they are two names, or they are one name whose digits reach fuller coverage than an 11 px semibold label's stems do. No capture in the reference separates the two.
...
627. **[decide]** **The vault's sidebar rows carry no symbol in a fixed column.** MEASURED by the fresh eyes: every row in `voicememos-multi-folder-2026-09-18.png` and in `finder-window-untinted-dark.png` draws a glyph in a fixed 20 px column with the label beginning 48 px in from the panel's leading rim — waveform, trash, folder, clock, app icon, tag dot. vaultview's rows are wording alone and its disclosure chevron stands where the platform's symbol would, so the column reads as an outline rather than as a sidebar list. `patterns/sidebar` takes a symbol per row already; what a folder TREE puts in that column beside a disclosure is the decision.
628. **[decide]** **The vault's tree has no section labels and its indent column shifts.** MEASURED by the fresh eyes off `window-light.png`: "Sources" stands at x=47 on the same indent column as "Design" and "guide" while "Reading list" directly above it stands at x=61, so a reader cannot tell a sibling under-indented by 14 from a root returning to the top level. Both references part their groups with a section label — "My Folders" at `#7d7d7d` with 22 px of air above it in Voice Memos, three of them in Finder — and the tree has none.

...
## DA. From CG7.8, the sheet's footer band and a bordered control in a body

Filed 2026-09-23 from the measurements this task took off
`save-dialog-{light,dark}.png` and from the work itself. No fresh-eyes review
was named; the regenerated goldens were read against the capture by the
author.

803. **[note]** **The two push buttons on one sheet fit two different corners.** A circular fit to the per-row coverage of "Cancel" gives r = 6.11 light and 6.17 dark, all four corners agreeing to the hundredth; the same fit to "Save" beside it gives 5.50 and 5.51. The grey button's fill stands 19 of 255 off the sheet light and 16 dark, so a coverage read across it is linear on whatever transfer the capture stores; the blue one spans 234, where it is not. The grey reading is the one carried. Whether the remaining 0.6 px is the encoding alone, or the platform draws its default button a shade tighter, is unread and would settle on a capture of a wide grey button beside a narrow blue one.
804. **[note]** **A segmented control standing in a sheet's body draws its chosen segment with the TOOLBAR's measured checked overlay.** `tokens.PlatformColors.ToolbarCheckedOverlay` and the patch insets beside it were read off Finder's view control in a band 36 px tall. Nothing in the reference holds a form segmented control at all, so what the platform fills the chosen segment of a dialog's own with — the accent, as macOS 11 onward draws it, or a step off the push button's fill — is unmeasured. The capture is a System Settings or Finder Get Info sheet carrying one.
805. **[note]** **A bordered control in a body keeps the capsule the chrome path draws, where the form trigger draws the button's rounded rectangle.** `components/button`'s `ChromeSegments` and `ChromeFace` corner at half the control's height and take no radius scale, so a caller that names the body place gets a 24 px capsule beside fields cornered at the measured 6. Giving them the scale is a signature change across every caller, which is why this round did not take it.
806. **[note]** **The symbol box a bordered control hands its mark does not follow the control's height.** `control.ChromeMarkDp` is 24 — the size the icon set draws the platform's measured 17-19 px symbols at — and a band control 36 px tall leaves 6 clear a side around it. At the body's 24 the box is the control, and what clearance is left is the glyph's own 19 of 24 units. No stored capture holds the platform's own `+`/`−` pair under a list, which is where that number would come from.
807. **[note]** **What a dialog leaves between its body and its footer hairline has no reading.** The capture's own air there is 69 rows of a Save panel's accessory slack, not a measured gap, so `patterns/modal` spends the S3 it already puts between the header and the body. A capture of a sheet whose form fills it to the footer would settle it.

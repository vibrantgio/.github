...
728. **[decide]** **The one stored picture of the switch dialog cannot show the state the dialog opens in.** `window-switch-{light,dark}.png` is a static render — `modal.Render` over resolved tokens, no router — so `gtx.Focused` answers false and the band the live dialog opens wearing stands in no golden anywhere. The pictures now show the selected row and an unfocused list, which is a state no reader meets on open. Either the static render path learns to be told which tag holds the keyboard, or the plan states that focus is read off live frames only and the goldens are pictures of the resting composition.

729. **[note]** **`list.Halo` takes one fill for both halves of the band.** The half past the box and the half over it are both `focus.Ring(p, standsOn)`: the list paints no fill of its own and its rows stand on the surface it stands on, so one name is right everywhere except where a selected row reaches the box's own edge — which is exactly the opening state of both these dialogs at the top row. The indicator carries 127/255 coverage, so those two px composite over the window's fill where the selection's fill is what is actually there. A second colour would fix the top and foot and still not fix the sides of a selected row, which the band also runs down; the exact answer needs the band drawn per row-run, or the ring moved wholly outside the box as the Voice Memos toolbar field's is (`controls.md`, x 700 unblended).

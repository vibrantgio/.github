...
708. **[decide]** **The gallery's high-contrast demonstration set drives foregrounds and leaves fills.** `components/gallery`'s a11y page builds a set with `ControlAccent`, `Label`, `Text`, `ControlText` and `Separator` all opaque black on the light planes, to show what a caller may hand a component. `DefaultButtonFill` now carries the default action's fill, and that set leaves it — and `ControlAccent` beside it — at the platform's blue, so a filled button in the demonstration is blue where everything around it is black. Whether a high-contrast set drives a control's FILL as well as its foregrounds, and what the platform actually does under Increase Contrast, is unruled and unmeasured.
709. **[decide]** **`patterns/sidebar` exports the symbol column's numbers but no painter for it.** `SymbolBox`, `SymbolInset` and `LabelInset` are exported and `SymbolForeground` with them, but `drawSymbol` is not, so every caller that wants a mark in the measured column writes the offset arithmetic again: `workbench/vaultview`'s rail did, and CG5.16's folder browser now does. `PaintSection` and `PaintCount` are exported for exactly this reason. Whether the symbol gets the same treatment — and whether a content list drawing in that column should be reaching into a chrome pattern for it at all, or whether the column belongs somewhere both can name — is unruled.

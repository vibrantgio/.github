...
821. **[decide]** **A feeds heading holding the keyboard cursor draws nothing.** The cursor can now stand on a section's heading — Left puts it there, Up and Down walk through it — and the heading takes no pill, so the reader watching the accent pill sees it vanish and come back two rows later. The rows' own drawing was outside the task's boundary. Decide what a heading wears while the rail holds the keys and the cursor stands on it: the sidebar's pill at the heading's own height, the disclosure alone in the accent, or something else measured off the platform.
822. **[note]** **Left on a note collapses its parent folder, where the platform only moves to it.** The owner's result states it twice — a row inside an open folder collapses the folder and lands on it, and a note does what its parent answers — so vaultview's tree collapses the parent from a note while an NSOutlineView leaf would only walk to it. A collapsed folder does walk to its parent without closing anything, so the two cases differ. Recorded as the deliberate reading, not a defect.

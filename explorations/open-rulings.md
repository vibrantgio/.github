...
465. **[decide]** **"syntax palette" and "syntax base" both name the colours a fence wears.** `markdown/README.md`, the plan and now the gallery's caption say syntax base; roughly a dozen comments in `markdown/style.go`, `markdown/highlight`, `theme/brand` and `components/gallery/inventory` still say syntax palette. One word or the other, everywhere; the rename is comment-only outside `brand.Base`.

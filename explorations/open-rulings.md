...
632. **[decide]** **The platform's toolbar label tone has no name in the set.** MEASURED by the fresh eyes off `finder-window-light.png`: the title "Applications" and every enabled toolbar glyph — the ellipsis, both view segments, the grid and the magnifier — bottom out at exactly `#4d4d4d`, 132 pixels of the title sitting at that value; the same window's list rows read 39 and its sidebar labels 25. `finder-window-untinted-dark.png` draws its title and glyphs at 232/233. So the platform puts toolbar content BELOW body strength, and `#4d4d4d` is neither `labelColor` (black at 0.85, 38 over white) nor `secondaryLabelColor` (black at 0.5, 127). `workbench/vaultview` draws its band's name in `Text` — black — and its marks in `ControlText` at 39; the gallery's toolbar controls do the same.
...
634. **[bug]** **The light segmented control's seam misses its measured pixel by twelve.** MEASURED, `finder-window-light.png`: the back/forward pair's seam is the single column at x=362, `#f2f2f2` over the control's `#ffffff`. `separatorColor` over that fill resolves to `#e6e6e6`, twelve of 255 short. The dark capture's `#3a3a3a` over `#262626` lands within one of the same name. `components/button` draws the name, which is the Language's rule for the line where two flush regions meet, and carries the light miss; it is the same shape as `toolbarSearchRim`'s three-level miss, which was recorded as a pixel of its own instead.
635. **[note]** **No stored capture holds a search field reporting a count.** `mail-find-light.png` and `mail-find-dark.png` hold Mail's find highlight in a message body and no find bar at all, so the count's inset, its size and its colour could not be measured. Where it stands is composed from two readings that are: the clear mark's disc ending 14 clear of the field's trailing edge and the magnifier standing 13 clear of its leading one, both `voicememos-multi-folder-search-2026-09-18.png`. It takes the field's own body style and the field's own secondary label. One capture of a find bar closes it.
636. **[decide]** **A toolbar control's shadow plateaus where the platform's ramps from its first row.** MEASURED by the fresh eyes off `window-light.png` and `finder-window-light.png`: below the vault switch ours reads a flat 238 over y 44-53 on a 247 fill and only recovers by y=75, where Finder below its share capsule reads 244 at y=44 and is back to 250 by y=59 — six levels over sixteen rows, smoothly, from the first row; ours also reaches upward, 242 at y 6-7. The depth is right (nine against the recorded eleven at its deepest) and the shape is not. It is pool 622's finding — one rectangle sunk below a shape draws its peak flat — read on a toolbar control rather than on the sidebar panel.

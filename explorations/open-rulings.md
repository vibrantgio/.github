...
## CN. From CG5.7, the icon set's keyline and weights

Filed 2026-09-18 from the measurement itself and from the fresh-eyes review of
the set's whole row at 24 px in both schemes beside Finder's own light toolbar
band, Voice Memos' dark one and Voice Memos' sidebar. The task's own result —
a square keyline of 19 measured units, a second band of 0.93 on a period of
2.20, the chevron pair and the single chevron as marks, the toolbar's own label
colour and the segmented control's own seam — is recorded in `controls.md` and
in `nscolors.tsv`. The review's three ranked findings were acted on inside the
task: the three large chevrons took the capture's 8 by 14, the folder's lid
rule took the capture's inset that tells it from the open folder's, and the
magnifier's handle stopped crossing into its own lens.

662. **[decide]** **The set's round and curved allowance is not measured and the square keyline now is.** CG5.7 moved the square keyline from 18 units to the platform's measured 19 (Finder's tag 19 by 19) and redrew the three chevrons at the 8 by 14 `finder-window-light.png` reads them at. What is left on the unmeasured 20-unit allowance is `search`, `clear`, `check` and `refresh`. The platform's own round and curved band symbols measure 16 by 17 (Finder's magnifier) and 17 by 17 (Notes' compose), so our magnifier is 20 by 20 where Finder's is 16 by 17 and carries about half as much ink again on its ring — 100 px² against 66. The allowance is this set's own optical compensation and the ruling that moved the keyline did not reach it.
663. **[decide]** **The set's own strokes span 1.15 to 2.13 px where the platform's span 1.30 to 1.51.** MEASURED by the fresh eyes as alpha-integrated coverage perpendicular to each stroke at 24 px: the chevron pair 1.15, the four axis-aligned figures 1.35, the sidebar 1.44, `clear` 1.77, `check` 1.9, the two rings 1.99, the three chevrons 2.13. Finder's own strokes on the same white, in the same ink, read 1.30 to 1.51 (grid rule 1.30, grid cell edge 1.34, pop-up chevron 1.44, magnifier ring 1.51). The set holds two numbers by rule — 1.5 axis-aligned and 2 diagonal — and the second is deliberate: this backend composites in linear light, where a 45-degree band at 1.5 units arrives at 67 per cent of the colour. So the spread is the compensation working as designed, and it is also what the review saw: in one row at one size in one colour, five marks read black and five read grey. Whether the compensation should be smaller, or spent per figure rather than per direction, is unruled.
664. **[decide]** **One square keyline cannot serve both square readings.** The reference holds Finder's tag at 19 by 19 and its group pull-down at 18 by 18, and Mail's and Notes' compose — the platform's own new-item control, which `plus` is read against — at 16 by 16 and 17 by 17. The keyline is 19, so the plus mark is drawn two to three units wider than the symbol it was measured from, and the folder and the page are drawn to a keyline no reading of THEM gives. Whether a set drawn at one keyline is right at all, or a symbol should carry its own measured extent, is unruled.
665. **[note]** **The 19-unit keyline centred in a 24-unit box touches 20 device columns at 24 dp.** Nineteen is odd against 24, so a centred square form stands at 2.5 and 21.5 and its two outer columns are half lit. The platform's 19 px tag covers 19 columns whole. The alternative is an integer keyline half a unit off the box's centre, which every placement in the library would then carry. Recorded against the ruling that chose the 19.
666. **[note]** **No stored capture holds an ENABLED light history chevron.** The pair in `finder-window-light.png` is drawn in `disabledControlText` — Finder stands at the top of its history there — so the 8 by 14 the three chevrons are now drawn to is measured and its colour is not. One capture closes it: a **frontmost light Finder window with history behind it**, window-bounded at 1x with wallpaper tinting off.
667. **[note]** **No stored capture holds a toolbar title or glyph on a light band that is not white.** `toolbarLabel`'s light `#4d4d4d` is read off `finder-window-light.png`, whose band is the content's own `#ffffff`. The dark half is proved opaque — the title on the `#1e1e1e` band and the glyph on a `#262626` fill read one 255th apart, which no coverage over two fills gives — and the light half has one fill only. The capture `controls.md` already asks for under "What the toolbar control measures" closes it: a **frontmost light window whose toolbar band carries the sidebar material under a bordered control**, window-bounded at 1x with wallpaper tinting off.
668. **[bug]** **The gallery carries no icon row.** CG5.7's own review step asked for "the gallery's icon row at 1x in both schemes"; `components/gallery` draws no page holding the set, and `icons.Names()` is walked by the icons package's tests and by `workbench/iconbrowser` and by nothing else. The sheet the review was given was rendered from a throwaway harness in the test package and thrown away with it. A set whose whole point is one grid at one weight has no place a reader sees all of it at once.
669. **[decide]** **`folder` and `open-folder` are two closed folders.** Both captures hold a CLOSED folder — Voice Memos' sidebar row and Mail's folder pull-down — so the two marks differ only in what those two drawings differ in: a lid rule inset 1.25 units against one running the full inner width, a tab 6.5 units wide against one 7 wide, and that rule standing one unit higher. The fresh eyes could not tell them apart before the inset was drawn and had to diff the arrays. The name says the control OPENS a folder and no stored capture holds an open one. Pool 607 already holds the name; this holds the drawing.
670. **[decide]** **The refresh mark's arrowhead reads as a nub.** The head is a triangle 6 units tall and 5 long capping the ring's end face at x=12, which at 24 px is three rows of ink above the band and three below it, and the fresh eyes read it as "a lump on top of a circle" with one barb rather than an arrow. `refresh` has NO capture anywhere in the reference — none of the stored toolbar bands carries a reload control — so the figure is drawn to the grid alone and there is nothing to correct it against. One capture closes it: a **window whose toolbar band carries a reload control**, window-bounded at 1x in either appearance.
671. **[note]** **The check mark is 18 by 13 where the rest of the set fills 19 or 20.** It is the shortest mark in the set and sits visibly smaller than the `clear` mark beside it on the same centre line. `check` is NOT MEASURED — no stored capture in `reference/macos` holds a checkmark at all, in a menu or in a list — and the capture that would settle its proportion is already on the reference's list.
672. **[note]** **A 45-degree band of 2 units lands no whole device pixel at 20 dp on the chevrons' phase.** A band of width w at 45 degrees covers a whole pixel only where a pixel's centre stands within w/2 − √2/2 of its centre line; at 20 dp two units is 1.667 px, which leaves 0.126 px, and the chevrons' centre lines put every pixel centre 0.354 px off. Their best pixel is 96 per cent covered and arrives at `0x39` against the `0x14` the set's evenness rule asks for. The 11 by 20 figure the set drew before CG5.7 crossed more phases and landed one; the 8 by 14 the capture gives does not. `components/icons`' own test states the arithmetic and the measured byte.

...
531. **[bug]** **The published sheet still draws the picker's pre-CG5.1 trigger, and `design`'s mirror has been red since CG4.21.** MEASURED on the authoritative machine: `dropdown.html` scores 0.0364 against its 0.0265 ceiling at CG4.21's head, because that task moved the trigger's insets in Gio and left `styles.css` alone. Giving `.select` the pop-up's own geometry — the control height exactly rather than a floor, no border until focus, the label 11 px in, the 8 × 11 chevron pair 9 px clear of the trailing edge as a mask so its colour stays a platform name — takes it to 0.0271, which still fails. The sheet's change is written and measured and was not committed, because a module cannot be committed red.

532. **[bug]** **`theme/typeset` ignores `TextStyle.Tracking` while the published sheet applies the same token as `letter-spacing`.** MEASURED: "Comfortable" set in BodyLarge runs x 12–99 in the Gio capture and x 11–104 in Chromium's — five columns over eleven characters, which is the 0.5 px tracking BodyLarge declares and Gio never spends. It is most of what `design/mirror`'s "cross-renderer floor" is made of, and it is a drawing difference in every label the library sets, not only in the mirror.

533. **[decide]** **`design/mirror` can no longer tell its dropdown pair from a wrong variant.** `pushButtonLabelFloor` is 0.0265; the calibration's nearest wrong variant scores 0.0279; the corrected pair measures 0.0271. Any ceiling that passes the pair stops being a gate. It closes with 532, or with the normalisation `pushButtonLabelFloor`'s own comment already names — distance divided by the frame's foreground-to-fill range — and not by moving the number.

534. **[bug]** **The form trigger has no hover and no pressed state at all.** `FieldState` carries `Focused` and `Disabled` and nothing about the pointer, where the chrome variant tints under both; the gallery therefore shows neither, and the trigger of an OPEN field is drawn pixel for pixel as a closed one. MEASURED, `control-hover-{light,dark}.png`: the Finder toolbar's VIEW POP-UP under the pointer reads `#f2f2f2` light and `#384146` dark against `#ffffff` and `#242d32` at rest, which is `hoverOverlay` exactly — so a pop-up does take the overlay and this control is simply missing it.

535. **[seed]** **No stored capture holds a PRESSED pop-up.** `control-pressed-{light,dark}.png` differs from `save-dialog-{light,dark}.png` only over x 359–432, y 501–524, which is the "Cancel" PUSH BUTTON; the pop-ups in the same sheet are at rest. Pressed is the state a pop-up spends its whole interaction in. Joins the capture list at 425.

536. **[decide]** **The platform's menu marks do not scale with the control, and `toolbarface` models the pull-down's as a ratio of it.** MEASURED: Mail's folder pull-down draws 9 × 5 in a control 29 px tall (`mail-window.png`, x 740–748, y 23–28) and Finder's group pull-down 8 × 5 in one 36 px tall; the pop-up's pair draws 8 × 11 in both the dialog's 24 px control and the toolbar's 36 px one. The platform sizes these by point size. `chevronWidthRatio`'s 9/29 gives 7.4 px at our 24 px control where the platform draws eight or nine, and the same question stands over the pair's own ratios, which CG5.1 wrote the same way to keep a proportion at a density nothing is captured at.

537. **[decide]** **The two variants draw their marks in two colours and only one is measured.** MEASURED: the dialog pop-up's pair flattens `labelColor`'s 216 of 255 onto both fills to the byte — 36 light, (224,225,226) dark — where `secondaryLabelColor` would land at 118 and 163, so CG5.1's form trigger spends `ControlText`; `toolbarface` spends `SecondaryLabel` for the pull-down's chevron. The review read the consequence off the drawn pixels: the chrome mark peaks at grey 124 against its own label's 38, where Finder's toolbar chevron peaks at 77 level with the icons beside it. The toolbar's own chevrons read neither coverage exactly (0.698 over white), which is that band's vibrancy rather than a third colour.

538. **[decide]** **The platform's toolbar picker wears the pair and ours wears the pull-down's single chevron.** MEASURED, `finder-window-light.png`: the view pop-up — a pick-one control — draws the stacked pair at x 726–733, and the group pull-down beside it — a menu of actions — draws one chevron at x 792–799. `components/picker`'s chrome variant is a pick-one control drawing the single chevron, which on this platform says "a menu of actions opens here". The two glyphs are the platform's own distinction between the two controls and the library spends them the other way round for the chrome variant.

539. **[decide]** **The chrome variant's trigger is figure where the platform's is ground.** MEASURED: Finder's toolbar pop-up is a filled capsule lighter than the band it stands in — white on a 250–252 toolbar with a soft shadow and no stroke — and is 49 px wide at 24 tall with a rounder corner than the dialog's. Ours draws no fill at rest and a one-dp hairline, which in the dark appearance is a (50,50,50) line on (30,30,30). `toolbarface.Fill` returning nothing at rest is the deliberate half; the platform's resting fill is the half nothing has been measured into.

540. **[decide]** **A disabled control fades its wording and keeps its whole bezel.** `components/picker`'s trigger draws the same fill disabled as enabled — byte-identical in both appearances — and only the label and the mark fade. The platform draws a disabled control's bezel at a reduced opacity too; the save dialog's own disabled checkbox is the one control in the reference that shows it, and the reference records that its fill moves and its extent does not. No stored capture holds a disabled pop-up or push button, so the number is owed with 535.

541. **[decide]** **The label a pop-up carries is a fifth larger than the platform's inside the same bezel.** MEASURED: the platform's "Script" has a 10 px cap band and a 7 px x-height in a 24 px control, 0.42 of it; the library's BodyLarge gives a 12 px cap and a 9 px x-height in the same 24, 0.50. The bezel is the platform's and the type inside it is not, which is why the control reads as a web control at 24 px. Its two companions: the chrome variant is set in LabelLarge and the form variant in BodyLarge, so one control shows two label sizes across its own specimens; and the label's cap band centres a pixel above the control's centre row where the platform's centres exactly, which is the line box's own centring and every control in the library shares it.

542. **[seed]** **The mark is stroked where the platform's is a tapered glyph.** The pair is drawn as two polylines at a uniform 1.5 px, and the platform's arms narrow toward their tips, so at 1x ours reads a little heavier at the shoulders than the platform's two thin strokes meeting at a point. Size, gap, stroke and placement all measure the platform's; only the profile does not, and a tapered outline rather than a stroke is what would close it.

543. **[decide]** **At Compact the form trigger draws 19 while the line box it carries is 24.** The pop-up takes the control height and nothing else, which is what the platform's 24 reads at Comfortable; at Compact the BodyLarge line box overflows the control by two and a half rows at each end. The glyphs fit — the cap band is about twelve — but a control no longer contains its own text box. It closes at either end: a smaller role at Compact, or a measured small pop-up.

...
819. **[note]** **The observable button reaches the bordered form symbol control when a caller wants it.** The Button entry says a symbol may be a button's label; with `Place` gone, an icon-only `Variant: Form` draws the borderless emphasis icon button, and the bordered form symbol control is reachable only through the pure `RenderBordered`. No caller in the org wants it yet. Engineering, not a ruling (ontology, 2026-09-24): no task until a caller does, and then `Props` names which form drawing it wants.
...
## DF. From CG7.14, the outline keys in the two rails

Filed 2026-09-24 from the work itself: feeds' headings joined the run its keys
walk so a heading can hold the cursor, and vaultview's tree learned Left and
...
## DI. From CG7.17, the two boxes at sk150's panel foot

Filed 2026-09-26 from the work itself: the hairline and the two-row table
under the readings became the OWON's two bordered boxes, and the box rim was
read off that photograph. No fresh-eyes review was named for the task.

828. **[decide]** **The panel now holds a fourth colour it argued it could not hold.** `displayCaption` carries no hue because the SK150's photograph has three lit colours and no fourth to spend on a word, and the two box titles still take that brightness. The box rim does not: it is `#1D58E9`, read off the OWON's own box edges, a saturated blue lit on the display's black. The task named the rim's source, so the reading is not in doubt; whether a line drawn on the display may carry a hue the display's own code forbids a value is.
829. **[decide]** **The OWON lights both values in one colour and this panel lights them in two.** The photograph's four box values are one yellow-green, `#BDE45E` over 22982 glyph-core pixels; here the volts keep the volt line's green and the amps the amp line's yellow, so a value matches the reading it governs. Two readings of the same box: one says a setpoint is a setpoint whatever quantity it names, the other says the colour names the quantity. Which the boxes should say is unasked.
830. **[note]** **A pixel-exact whole-window golden reports the rasterizer's coverage as a change.** This task moved nothing outside the readout panel, and three pixels in the chart panels below it still differ by one step in one channel between the old goldens and the new — the same paths rasterized under a changed op stream. The patch comparisons in the same file already allow four steps for exactly this; the stored frames allow none. Whether a whole-window golden should carry that tolerance is unasked.
831. **[note]** **The panel writes a title and the code calls it a caption.** `setLimitLine.Caption` and `Palette.DisplayCaption` are what the two boxes' first lines are drawn from, and the prose around them now says title, which is what the boxes hold. The two names are the panel's alone and no other window reads them. Whether they should follow the word is unasked.

## DJ. From CH1.1, the bottom layers' tags

Filed 2026-09-26 from the work itself: the layer-0-to-2 modules took the
round's tags. No fresh-eyes review was named for the task.
...
834. **[note]** **`mvu/example` sits a layer above the round that tags its siblings.** It imports `theme`, so it derives as layer 3 and falls outside CH1.1; CH1.2 names theme, effects, components, patterns, markdown and the gallery, and CH1.3 names workbench, so no Phase CH task names it. Its `go.mod` still pins `backdrop v0.0.3`, `circle v0.0.4`, `mvu v1.0.0`, `ivg/raster/gio v0.1.7`, `textdraw v0.0.4` and `theme v1.3.0`, and `example/v1.0.0` stands three commits behind master. Whether the second pass should carry it is unasked.

...
437. **[bug]** **The platform draws neither seam we draw between a sidebar and the content.** MEASURED, `finder-window-untinted-light.png`: the sidebar's #f7f7f7 runs to x=351 and the content's #ffffff begins at x=352 with NO hairline between them, in 112 of 114 sampled rows — the two are told apart by the shade alone. Dark, `finder-window-untinted-dark.png` draws one pixel of #434343 at x=373, which is neither separatorColor over the chrome material (#323232) nor over the content (#343434) nor any other name in `nscolors.tsv`. We draw separatorColor over the fill in both schemes, per the Language's Seam entry, which is a visible rule where light has none and the wrong value where dark has one. One ruling: the Seam entry's rule, or the two measurements, and a name for #434343 if the measurements win.

438. **[decide]** **`patterns/pane` no longer draws an inset object at all.** CG1.1's second pass made the pattern the flush chrome column — the run to the window's leading, top and bottom edges and one seam — because that is what Finder's untinted captures measure. Nothing in the checkout wanted the inset kind: the only callers were vaultview's rail, mindchat's rail and the gallery's own specimen, and all three are rails. The rounded inset, its four-sided hairline and `FillTrailingCorners` were removed rather than parked under a second name. If a later pattern wants an object set into the window's plane — a detached inspector, a floating palette — it is a new pattern and not this one's second mode.
...
## CI. From CG5.3f, the toolbar control's shadow and a symbol's capsule

Filed 2026-09-18 from the measurement itself and from two fresh-eyes rounds on
mindchat's toolbar beside the Finder band in both appearances
(`reviews/cg5.3f-toolbar-fresh-eyes.md`). Both rounds read the chassis as the
platform's — the 36 px band, the capsule at half its height, the dark rim, the
shadow doing the work in light. Three of their findings were this task's own
and were fixed rather than filed: the two chrome figures drawn at two weights,
the shadow cut off at the chrome row's foot by the transcript painted over it,
and both figures standing a pixel below their control's middle.

577. **[decide]** **Two applications draw a toolbar mark standing over a SIDEBAR bare and one draws it bordered.** `notes-window.png` and `reminders-window.png` both carry a new-item mark and a sidebar toggle over the sidebar region with no capsule, no fill and no rim, where every control over the CONTENT in the same two bands is bordered; `voicememos-window.png` puts its sidebar toggle in a full capsule over the same kind of region (x 96–135, fill `#272727` on a `#1e1e1e` band, the dark rim at either end). CG5.3f followed Voice Memos, the application the Language already names for the toolbar search field, and bezelled both halves of mindchat's switch. The platform does not settle it and a ruling would.
578. **[bug]** **`vaultview`'s two sidebar toggles are still bare chrome marks.** `frame.go`'s chrome-row toggle and `tree.go`'s rail-strip toggle draw `icons.Sidebar` at `railToggleMarkDp` on the band with no control around them, which is what CG5.3f took out of mindchat. They cannot take the bordered control as they stand: that window's chrome row is `toolbarHeight` — one LabelLarge line box plus two of the smallest spacing step, about 28 dp — against the toolbar control's measured 36, and `onButtonLine` hangs its content below the row's foot. The row's own height has to be measured first.
579. **[capture]** **No stored capture holds a toolbar band with room ABOVE a bordered control.** The shadow's reach over the control in `finder-window-light.png` is cut off by the window's own top outer edge eight rows up, where the band still stands two 255ths down; the 15 rows `controls.md` records are extrapolated from the profile below it. A window whose toolbar band is not at the top of the capture would close it, as would the light band that is not white the reference's earlier open question already asks for.
580. **[decide]** **The dark toolbar shadow is drawn through the light one's geometry.** MEASURED, the dark shadow is one 255th deep and seven rows tall, below the control only; the light one is eleven deep and reaches 39 rows below, 31 columns beside and about 15 above. `tokens.PlatformColors.ToolbarControlShadow` carries a coverage per appearance and `components/internal/control` carries ONE reach and ONE offset, so the dark drawing spreads its one 255th over a halo the platform keeps to seven rows. The miss is one 255th on a band that dark and is recorded; a reach and an offset per appearance would close it, at the cost of two more numbers on the scale.
581. **[bug]** **A chrome spring button silently draws the form button.** `effects/springbutton` now forwards `button.Props.Variant` into its `RenderState` — its own reflection test demands it — but it renders through `button.Render`, the TEXT path, which ignores the variant by contract. A caller that asks a spring button for the chrome variant gets the form one with no warning.
582. **[decide]** **The Language's Variant table names the picker alone.** DOMAIN.md's Variant entry lists `form | picker` and `chrome | picker`; `components/button` now carries the same two, and the search field has carried them since CG4.8. The table is a list of variants by component and three components now share one pair.
583. **[decide]** **The new-chat symbol is an application's own drawing.** `components/icons` carries sidebar, disclosure, history-back, history-forward, search and clear, and no plus; mindchat draws its own `PlusGlyph` beside the set's own weight and keyline because the set has nothing to ask for. Every application that offers a "new" action will draw its own.
584. **[decide]** **A chrome control that travels between two places moves under the pointer.** Both fresh-eyes rounds read it: mindchat's sidebar toggle and new-chat control stand at the pane strip's trailing corner while the pane stands and 51 px further leading in the content area's row once it is away, so pressing the toggle moves the control that was pressed. It is the recall convention `frame.go` states — a control that travels with the pane cannot be the one that recalls it — and two stored captures put an application's sidebar-side marks at that sidebar's trailing end the way this window does. Whether one window may move a control between two places is the application's question.
585. **[decide]** **A chrome figure's band cannot carry the platform's apparent ink and its crispness at once.** The platform's straight symbol bands read 122 and 147 on a `#ffffff` fill — a 1.12 px band split across two pixels, composited in the encoded byte. This backend composites in linear light, where the same split reads 141 and 141 and the figure has no pixel in the colour it was given; phased onto the pixel grid the same band reads 39 and 190, a core in the colour with a lighter fringe. The first fresh-eyes round read the unphased figure as half-greyed and the second read the phased one as inkier than the platform. `components/icons` documents the same gap for diagonals and compensates by drawing heavier; nothing yet says what an axis-aligned band should do.
586. **[decide]** **Two single-symbol controls stand apart where the platform would segment them.** The stored bands hold single-symbol controls standing alone (Mail's and Notes' compose, Voice Memos' sidebar toggle) and groups of two or three sharing ONE capsule with hairline seams (Finder's back-and-forward, its share/tag/more, Mail's reply trio), and no stored band holds two separate single-symbol capsules side by side. mindchat's toggle and new chat are two halves of two different switches rather than one group, which is why they are drawn apart; the segmented control the platform uses for a group is not drawn anywhere in this library.
587. **[decide]** **A chrome control that records a state has no on-state drawing.** The one state cue in the stored bands is Finder's segmented view control, whose active segment carries a lighter inner fill — 73 against the capsule's 38 in `finder-window-untinted-dark.png`. Nothing in this library draws it, and the sidebar toggle is pixel-identical whichever way its pane is set, which the first fresh-eyes round read as a control giving no clue what it is about to do.

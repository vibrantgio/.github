...
252. **[decide]** The three-column frame draws a divider only between
     ...
     missing leading divider is what a reviewer reads first.
     *(§AH, BR1.1's review)*
...
253. **[bug]** The three-column frame's footer strip is not yet a
     ...
     from the top of a 48 px strip rather than centred in it.
     *(§AH, BR1.1's review)*
...
256. **[task]** `design/DESIGN.md`'s elevation section still documents
     the model before BR1.1 — the lowest level as chrome furniture,
     four levels, ordered away from the desk. It is hand-written, not
     generated by `vg-tokens`, so it needs its own rewrite in the
     Language's terms: backdrop, chrome, then 0 to 3.
     *(from BR1.1's execution)*

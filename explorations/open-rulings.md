...
552. **[decide]** **An open picker's trigger has no treatment of its own.** Diffed against the closed trigger, the open one differs only in its label: the same fill, the same mark, the same corner. Held, the same control darkens by 23 levels; with its menu standing it darkens by none. The platform does not have to answer this because its pop-up's menu covers the button, which is the placement CG5.4 leaves open — so the question is whether the trigger owes a standing state at all, or whether it is answered by the menu landing where the platform puts it.

553. **[decide]** **The switched-off label is far weaker in the dark appearance than in the light.** One coverage is spent in both — the measured fade over the fill, the platform's `disabledControlText` over that — and the two land 48 Lc apart: the picker's switched-off wording reads Lc 31.7 light and Lc −16.7 dark. The coverages are the platform's own in both appearances and the asymmetry is the platform's arithmetic, not a defect in the drawing, so what is open is whether a contrast floor applies to a switched-off control at all.

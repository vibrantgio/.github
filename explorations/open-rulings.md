...
544. **[bug]** **The two pointer overlays leave a dark control's hovered and held states one 255th apart.** `hoverOverlay` dark covers 24 of 255 and `pressOverlay` dark 25, so a dark trigger reads `#464d51` under the pointer and `#474d52` held, and a dark accent button `#1887ff` and `#1987ff`; light draws the same pair eleven levels apart. A held dark control therefore gives no feedback, and a pointer merely crossing one shouts as loudly as a press — the fresh eyes ranked it worst by a wide margin and found it independently. Both coverages reproduce their own captures on every channel, so neither is wrong on its own; what they were read off differs — hover off a Finder toolbar control carrying NO fill at rest, press off a push button over its own fill — and nothing says whether the platform draws a weaker hover on a control that already carries a bezel. One dark control captured under the pointer and then held settles it; it is on 425's list. The reviewer's proposed midpoint is in no capture, so nothing was painted from it.

545. **[bug]** **A switched-off text field reads as an absence in the light appearance.** Its interior is the surface it stands on by measurement, so there is no body to fade, and all this task could fade is the hairline — from twelve levels off white to eight. What is left saying the field is off is the prompt alone, where the platform's own switched-off control in the reference keeps a thirteen-level body. No stored capture holds a switched-off text field, so the rule cannot be corrected from the reference as it stands.

546. **[decide]** **`picker.FieldState` carries no `Surface`,** so a switched-off form trigger fades toward the window's plane wherever it stands — on a card, a selected row or a coloured fill it fades toward a colour it is not on. `components/button`'s `RenderState` and `components/input`'s both carry one for exactly this.

547. **[decide]** **The gallery shows no control under the pointer for the picker or the text field.** Four field cells at 160 dp plus the toolbar trigger already fill the 900 px page, so the form trigger's two new states could not be added to the row and were recorded as `components/picker`'s own goldens instead; the chrome trigger does take the overlay and has carried hovered goldens since before this task, but the gallery's row shows it at rest only, so a reader of the page cannot see a state that is drawn. Whether a text field tints under the pointer at all is unmeasured.

548. **[bug]** **`theme/export`'s CSS sheet is behind the components by two tasks.** It emits no hover rule and no fade on a switched-off fill, and its `.select` still draws the hairline, the 16 dp width and the solid triangle CG5.1 deleted. CG5.2 corrected the sheet's comments, which claimed the missing hover rule was a measured absence; the rules themselves are owed.
...
550. **[bug]** **A sidebar row's and a list row's exemption from the hover overlay is asserted and not measured.** `patterns/sidebar`, `workbench/mindchat` and `workbench/feeds` each carry a comment saying the platform does not tint such a row under the pointer, and `theme/tokens` carried the same claim for a list row until CG5.2 deleted it. No stored capture holds either row under the pointer. Same class as the push button's exemption, which CG5.2 deleted for want of a capture.

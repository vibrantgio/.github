...
## CN. From CG5.4, the picker's open menu as the platform's pop-up

Every item here is one capture away from an answer, and it is the same
capture: a pop-up button with its menu open, in both appearances, which
`reference/macos/controls.md` records under "Also open: the open menu" and
425's list carries. They are filed apart because they are different readings
off that one frame, and because each names what CG5.4 drew instead.

644. **[bug]** **The open menu's plane is the window background, the same value as the window behind it.** No name in the recorded set is a menu background, so `components/picker` takes what the Language gives the floating level and lets the shadow tell it — which in dark is one to two of 255 deep, so the fresh eyes read the panel as a hole cut in the window rather than a surface over it. The platform's menu material is lighter than the window it covers; nothing here measures how much.

645. **[decide]** **The menu's selection pill is the sidebar's, inset 10 and cornered at 8**, because `patterns/sidebar`'s is the one pill this reference measures. A menu's own highlight is inset less and cornered shallower, and is shorter than the row it stands in; all three are unmeasured.

646. **[decide]** **The menu's check gutter is 32 px** — the pill's inset 10, the icon set's smallest box 16, and the text field's leading inset 6 — every part named from something measured elsewhere and none of it off a menu. The platform's is narrower. Its row height is the library's stacked-row rule rather than a menu row's, which is the same reading.

647. **[bug]** **The open menu's plane has no vertical padding**, so the first and last rows' pills stand against the plane's own corner and fight it. The platform leaves a margin above the first item and below the last, which is what lets those two pills sit inside the corner. Any number for it is invented until the capture exists.

648. **[bug]** **In the light appearance the menu's hairline and the deepest pixel of its own shadow are both `#e6e6e6`,** so the edge is invisible and the plane reads as shadow alone; dark draws a visible hairline and a shadow of one to two of 255. One construction, read by the fresh eyes as two. `Separator` over the plane is what puts the light line on the shadow's own value.

649. **[decide]** **A floating surface's shadow is symmetric with no downward offset**, where the platform's is lit from above. The reference already records this as the limit of one rectangle with one peak, for `PaneShadow` and `ToolbarControlShadow`; CG5.4's fresh eyes read it on a menu, where a symmetric ramp reads as an elevation glow rather than a menu's shadow.

650. **[decide]** **Whether a menu's selection is the sidebar's pill colour at all.** `SidebarSelection` is what CG5.4 drew, being the one selection measured inside a chrome region; `SelectedContentBackground` is the content list's and is the other candidate. The reference states the pill is neither `controlAccentColor` nor the content list's, but says nothing about a menu.

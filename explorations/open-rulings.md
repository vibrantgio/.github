...
492. **[decide]** **The search field's magnifier is a bold icon where the platform's is a hairline.** Measured off the rail beside the capture: ours occupies 16×16 px with a 12 px circle drawn at a solid 2 px stroke and a 5 px handle 2 px thick; the platform's occupies 14×13 px with an 11 px circle at ~1 px and one antialiased neighbour, its handle ~3 px at ~1.5. Read cold as "the single most visible difference, and the one thing that gives our field away in both appearances". Ours also centres the glyph's whole bounding box, which lifts the circle 2 px above the field's centre where the platform centres the circle and lets the handle hang below it. Both are the icon set's asset and the density's icon size, so both reach every mark in the library, not this field.

493. **[decide]** **The stored captures give three leading insets for three search fields, and the library draws a fourth.** The field's edge to the first pixel of the magnifier: 9 px in System Settings' sidebar, 10 px in Mail's toolbar, 13 px in Voice Memos' capsule. `components/input` spends `spacing.S3` — 12 dp — which puts the glyph's marks 14 px in, and the gap between glyph and prompt is 11 px against the sidebar's 6, so the whole leading cluster is 12 px wider than the platform's and the prompt starts at 41 px rather than 29. One padding serves every field in the library; whether a search field takes its own, and off which capture, is the ruling.
...
495. **[decide]** **The dark recess carries a wallpaper tint our chrome does not.** `#2f3234` is blue-cast — b−r of +5 — because it was read off a capture with "Tint window background with wallpaper colour" on, over a `#1c2124` sidebar that shares the cast. Our rail is the untinted `#1c1c1c`, so the recess reads faintly cool on it. `CardFill`'s `#2a3034` was recorded the same way and sits on the same neutral planes, so this is the whole set's question and not one field's: either the recorded fills are neutralised to the material they will actually stand on, or an untinted dark capture replaces them. `controls.md` names the capture that would close it.

496. **[decide]** **The Language says the recess is a shade darker than the material, and in the dark appearance it is lighter.** DOMAIN's Search field entry reads "a flat recess a shade darker than the material". Measured: light, the recess is 18 levels darker than the sidebar; dark, it is 19 levels lighter. The direction does not survive the scheme, which is also why the fill is recorded as a colour rather than a step. The entry belongs to the ontology session; this is the correction it needs.

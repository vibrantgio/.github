...
482. **[decide]** **`workbench/sitedocs/theme_palette.go` keeps `PaletteRows`, `PaletteSectionRows` and `paletteChrome`, unrenamed by CG4.2.** These local identifiers draw the same board CG4.2 renamed in `components/gallery/inventory` (`PlatformRows` → `PlatformColorRows`) and `components/gallery/palette`, but the ruling named only those two call sites, and this window's own row-builder functions are not consumers of either — they call `palette.Rows()` directly. Left as-is; whether the `PlatformColors`-in-identifiers convention should extend here is unruled.

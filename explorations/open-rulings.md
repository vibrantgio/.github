...
## CQ. From CG5.14, vaultview's vault-switch dialog beside the platform's Save panel

Filed 2026-09-22 from the fresh-eyes review of the dialog open over
vaultview's whole window in both appearances beside
`save-dialog-{light,dark}.png`. The task's own result — the footer's 74 × 24
buttons at the save dialog's measured 8 dp gap and 20 dp insets, the 24 dp row
pitch, the header's and the footer's 20 dp outer insets and the body's
foregrounds — the reviewer read back independently and found faithful; the one
finding this task covered, rows standing 8 px in from the columns the header
and the footer lead and end on, it fixed. Everything below is the browser's
rows, the modal pattern's own chrome, and a colour token. The reply is in
`reviews/cg5.14-vaultview-fresh-eyes.md`.

696. **[bug]** **The default button's accent is published where a capture measures it.** `ControlAccent` is `#007aff` in both appearances — Apple's published systemBlue — and that is what a Filled button draws. The Save dialog's own default button measures `#157efb` in BOTH appearances (`save-dialog-{light,dark}.png`, flat over x 441–514, y 502–523), and `patterns/sidebar` already spends that measured `#157efb` for its selection pill. So one window draws two blues that are meant to be the same colour: the fresh eyes read the dialog's Open button against the rail's selection behind it and named the disagreement without being told to look. Measured beats published; `ControlAccent` re-measures off the save dialog, both tsv copies and ADR-019 follow, and every Filled button in the organization moves with it.
697. **[bug]** **The folder browser's rows carry no symbol.** MEASURED by the fresh eyes: the platform's open panel puts a 16 px folder on every row without exception, and ours carries a name and an annotation alone — "strip those and you have a menu, not a directory". `components/icons` already draws the sidebar folder the tree rail spends. It is the browser's row, so it lands on both the dialog and the full-screen picker at once.
698. **[decide]** **The browser leads with `..` and roots its trail at a literal `/`.** Both are the filesystem's own spelling shown to a reader: the parent row is the POSIX name, and the breadcrumb's first segment is a blue clickable separator character. Neither appears in any macOS chooser — the platform gives the parent as the trail's own segments and names the root "Macintosh HD". What a folder browser calls the place above it, and what it calls the top, is one ruling for the browser.
699. **[decide]** **The annotation column holds two different things and aligns to neither.** MEASURED by the fresh eyes: the slot carries "142 notes", "18 notes", "1 note" and then ".obsidian vault" — a count and a kind in one column — and a folder with no notes carries nothing, which reads as missing rather than zero. Right-aligning the whole string leaves the numerals ragged (142 at x=749, 18 at 756, 3 at 762, 1 at 769), and with the names on the leading edge there are 397 px of empty fill between the two halves of a row at the dialog's width. Whether a row's trailing slot is one column or two, and whether a count aligns on its digits, is unruled.
700. **[decide]** **`patterns/modal`'s surface takes `RadiusScale.Lg`, 6 dp, where the platform's sheet corner reads about 17.** MEASURED by the fresh eyes off the reference. The radius is the pattern's for every dialog in the organization, and 6 is what makes the surface read as a card rather than as a sheet.
701. **[bug]** **The floating shadow is a symmetric halo with no vertical offset.** MEASURED by the fresh eyes around the dialog: the darkening is equal on all four sides (above 204→189, below 189→204, leading 198→188, trailing 184→198) over about 24 px — a shadow cast by a light source directly in front of the surface, which no platform draws. `effects/depth` and the `FloatingShadow` token are where it lives. The same reading says the reference sheet casts no shadow at all and is told apart by its hairlines instead, which is 700's and 703's question as much as this one's.
702. **[decide]** **A modal stands at dead window centre.** MEASURED by the fresh eyes: y 230–570 in an 800-tall window, centred on the whole window including the sidebar. A macOS sheet descends from the title bar and sits high, attached to the window it interrupts. Whether the library's modal is a centred dialog or a descending sheet — and whether that is one answer or a placement the caller states — belongs with the Modal entry.
703. **[bug]** **In the dark appearance nothing draws the dialog's edge.** MEASURED by the fresh eyes: the surface is `#1e1e1e` on a `#161616` window, eight levels apart, with no edge, and the shadow ring lands at `#141414` — two levels BELOW the window it is cast on, invisible at 1x. The platform's dark sheet is `#232a2f` on `#191a1b` and still carries internal hairlines. A floating surface the reader cannot find the edge of is the Level entry's "a floating thing by its shadow" failing in one appearance.
704. **[decide]** **The library's modal is landscape where the platform's open panel is portrait.** The surface is 75% of the frame clamped to 560 dp wide and hugs its content in height; at vaultview's window that is 560 × 340, against the reference panel's 370 × 356. No stored capture holds the platform's own OPEN panel, so nothing measures what one opens at — that capture is on 425's list — and until it exists a chooser's proportions are stated rather than read.
705. **[decide]** **A dialog's Cancel is Ghost across the organization and the platform's is a filled push button.** MEASURED by the fresh eyes: our Cancel is bare label on the dialog's own plane in both appearances, where the reference Cancel is a 74 px push button filled `#ececec` light and `#333a3f` dark with a hairline, 8 px from the default button. mindchat's settings dialog and this one both spend Ghost, and CG5.14's own task text states it, so this is one ruling for every dialog rather than a defect in one. It also makes a Ghost button's box invisible in a still, which is what made the reviewer read the footer's measured 8 px gap as 19.

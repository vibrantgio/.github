...
## CN. From CG5.3p, the sidebar row as a symbol, a label and a count

637. **[decide]** **A sidebar row's symbol is drawn stronger than the label beside it, and no platform name answers for it.** MEASURED off `voicememos-multi-folder-2026-09-18.png` and `voicememos-sidebar-light.png`, two captures of the same rail at different window positions: the folder symbol plateaus at exactly `#000000` over 45 and over 30 pixels where the label beside it plateaus at `#262626`, and `voicememos-sidebar-dark.png` reads a plateau of `#ffffff` against that capture's own `#dcdcdc` label. A vector mark is not text and takes no stem darkening, so those are drawn colours: 38 of 255 past `Label` in light and 35 in dark. The Icon entry has a mark drawn in the foreground of what it sits in, which is the label's own colour, and that is what `patterns/sidebar`, vaultview's tree and mindchat's rail draw; CG5.3p's fresh eyes read the dark rail as sitting back from its surface and named the symbols with the labels. Rule: whether the rail's symbols take a measured value of the sidebar of their own, as the pill and the count now do, or the Language's answer stands and the 38 levels are accepted.
638. **[bug]** **`patterns/sidebar` is still the flush rail the panel replaced.** The Sidebar entry has the sidebar set into the window as a pane, no seam parting it from the content, and CG5.3n measured that panel and drew it in `patterns/pane`. The sidebar pattern still fills its own 192 dp column with `SidebarMaterial` and draws a `Separator` hairline down its trailing edge (`drawTrailingSeam`), where the measured panel is 220 dp across between its own rims. So the rail the gallery shows and the sidebar the Language describes are two different things, and every consumer that wants the panel composes it itself.
639. **[bug]** **`workbench/feeds`' rail lists and does not stand in the row's columns.** It takes `patterns/sidebar`'s `RowHeight`, `SelectionInset`, `PaintSelection` and `SelectionLabel` and then lays its own row out beside them, so its names do not begin at the measured 48, it draws no symbol, and its trailing end is its own. CG5.3p named vaultview and mindchat; feeds lists the same way and was left alone.
640. **[decide]** **A count's trailing end is measured to its ink and drawn to its layout box.** The platform draws every count to x 266 or 267 against a panel rim at x=283 — 17 to the last covered column. `patterns/sidebar` places the count's LAYOUT box 17 in, and the digit's own right side bearing then puts its ink 19 in: the "2" of `light-sections.png` ends at x=173 in a 192 rail. Nothing in the library measures a run's ink extents, so closing the two costs a measurement path through the shaper that every trailing-aligned text in the org would want. Rule: whether a trailing alignment is to the box or to the ink.
641. **[decide]** **vaultview's filtered row puts its folder annotation in the count's column.** The Language gives a row's trailing end to a count — how many things the entry holds. While the find is typed in, vaultview draws the matched note's folder there instead, which is what tells two notes of one name apart; nothing else in the row can carry it. So the column the Language names for a count holds something that is not one, on exactly the rows the reader is reading fastest.
642. **[bug]** **The open-folder mark's comment and its drawing disagree about the flap.** `open-folder.svg` says the flap "stands one band clear of the body's inner top edge, which is the capture's own relation"; that inner edge is at 7.5 and the path draws the flap from 10.5, three bands clear. The `folder` mark CG5.3p measured off the sidebar reads 1.12 units of air and draws its flap at 9, one band clear, so the set now carries two folders whose flaps stand differently for no measured reason. The mark was not touched: its name is held in 607.
643. **[bug]** **The design bundle's hand-authored sidebar specimen pictures the row that was replaced.** `design/components/navigation.html` still describes "a 48 px `.sidebar-item-icon` column" whose label starts at the column edge and calls the pill "the two-step walk to `primary-400`", a Material derivation deleted in Phase CE. `theme/export/css.go`'s generated rules now carry the symbol in a 24 square 17 in, the label at 48, the count 17 in from the trailing edge and the heading's 42 dp block; the hand-authored page does not, and the sheet is untouched until the resync.

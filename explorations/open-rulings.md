...
689. **[decide]** **`workbench/feeds`' rail is still a flush column where the pattern is now an inset panel.** CG5.8 moved `patterns/sidebar` to the measured 220 dp and took its trailing hairline away, because the platform's sidebar is a panel whose rim and shadow are the boundary. feeds draws its own rail: 192 dp wide with a 1 px seam down its trailing edge, so in the light appearance the reviewer measured `#ffffff` rail, one `#dfdfdf` column, `#ffffff` content — one sheet of paper with a pencil line on it, where Voice Memos steps `#f9f9f9` to `#ffffff` across a rim and then ramps a 35 px shadow onto the content. Whether feeds becomes a pane as vaultview did, or stays a flush column, is unruled. The seam is also why the reviewer read feeds' count as ending 16 in from the rail where the platform's ends 17: measured from the rail's last column, as the platform's is measured from the panel's rim, it is 17.
690. **[decide]** **feeds heads its runs with an accordion's header where the platform heads them with a small label in air.** MEASURED by the fresh eyes: our "Tech" is a 10 px cap at regular weight peaking `#262626` — the most pronounced text in the column — in a 48 dp row of its own with a permanent 10×10 disclosure triangle at x 11–20 and its text on a third column at x=32, where the item symbols stand at 17 and the names at 48. Voice Memos' "My Folders" is an 8 px semibold cap peaking `#7d7d7d`, floating low in 42 px of air with no row, no triangle and its first column on the symbol's own 17; Finder's three headings read the same. `patterns/sidebar` already draws exactly that heading (`PaintSection`, `SectionStyle`, `SectionHeight`, `SectionInset`, `SectionBaseline`), so the drawing exists — what is unruled is that feeds' groups COLLAPSE, which a sidebar section does not, so adopting the platform's heading removes a control the window has.
691. **[bug]** **The accordion paints over the rail's own chrome material.** feeds fills its rail with `SidebarMaterial` and then lays `patterns/accordion` over it; the accordion paints its body in the content's own fill, so below the first row the light rail reads `#ffffff` over every column and not the `#f7f7f7` the rail painted. The window's own test already records the symptom in a comment. A chrome region whose fill a pattern inside it replaces is a defect wherever it happens, not only here.
...
## CS. From CG5.17, the dialog as the platform's sheet

Filed 2026-09-22 from the fresh-eyes review recorded in
`reviews/cg5.17-dialog-fresh-eyes.md`. One reviewer was handed mindchat's
settings dialog in both appearances beside `save-dialog-{light,dark}.png` at
1x, the recorded misreads, and one question: does this dialog read as a macOS
sheet, and what is wrong with it. Everything below is that reviewer's reading,
measured off the four images; none of it is inside CG5.17's stated result.

711. **[decide]** **The dark dialog does not part from what it covers.** The reviewer measured mindchat's dark sheet at `(30,30,30)` and the assistant bubble it overlaps at `(30,30,30)` — equal to the byte — with the shadow beside it at `(27,27,27)`, a three-level step, against light's sixty-level step plus a fourteen-level shadow. CG5.16 ruled the scrim is what parts a dialog from the window beneath it and the platform draws no rim; a PANEL over content that happens to carry the dialog's own fill is the case that ruling does not cover. Decide what a floating surface owes the surface underneath when the two are the same fill: a stronger shadow in dark, a measured rim, or the panel purpose taking a scrim of its own.

712. **[decide]** **Three of mindchat's four settings fields carry no label, and the one label there is stands 180 px from its control.** The reviewer: the name, base URL and key fields are bare, where the platform's sheet puts a right-aligned label column at x≈256 with an 8 px gap to each field; and "DEFAULT MODEL" sits at x 252–332 against the pop-up it names at x 512–772, styled as the all-caps section heading "PROVIDERS" is, so it reads as a heading for empty space rather than as that control's caption, and it sits 7 px low against the control's own centre. A form-layout question for mindchat, and possibly a pattern: nothing in the library states where a field's label stands.

713. **[decide]** **The provider pills are a segmented control with no chosen segment, drawn as four buttons.** Four detached 85 px pills with 6 px gaps, all four at the push button's own fill in both appearances, while the list beside them already shows which provider is open — the same state said twice with one saying nothing. The reviewer also notes they are pixel-identical to the Cancel button beside them, so four things that are not the action look exactly like the one that is. Decide whether that row is a segmented control (contiguous, hairline-parted, the chosen segment tinted, which is what the platform draws) and what a chosen one wears.

714. **[decide]** **A dialog has no footer band and no hairlines of its own.** The reviewer scanned mindchat's sheet at x=500 and found an uninterrupted slab from y=178 to y=589, against the platform's full-width hairlines at y=324 and y=479 carving three bands with a 62 px button footer at the foot. `patterns/modal` draws header, body and footer with S3 gaps and no line anywhere. Decide whether the sheet's bands are parted by a measured hairline, and whether the footer is a band with a fill of its own.

715. **[decide]** **The dark sheet is neutral under controls that are tinted.** Measured: mindchat's dark sheet and its field interiors are `(30,30,30)`, dead neutral, while the well `(42,48,52)`, the pills and Cancel `(51,58,63)` and the field borders `(44,51,56)` are all blue-tinted, and the platform's dark sheet is uniformly tinted at `(35,42,47)`, field interiors included. The reviewer's own inference is that the control values were taken from the platform and the surfaces were not. The same box also steps the wrong way between appearances: the providers well is 8 levels darker than a light sheet with no border pixel at all, and in dark it is lighter than the sheet. Decide which surface the dark dialog stands at, and whether a well is a recess in both appearances.

716. **[decide]** **The providers well's own ends are ragged.** The reviewer: the +/− stand 15 px above the well's bottom edge with no bordered footer group around them, the key row ends 50 px short of the two fields above it because the verdict disc and the refresh glyph fill the gap, the verdict disc at `(52,199,89)` is the most saturated pixel on the sheet — more than the default button — for a passive indicator, the refresh beside it is a bare grey outline with no control chrome, and the whole bottom-left quadrant below the well is empty while the two columns end at different heights. One composition question for mindchat's settings sheet.

717. **[decide]** **A dialog's footer buttons measure 100 where the platform's measure 74.** The reviewer read mindchat's Cancel at x 564–663 and Save at x 672–772 against the platform's x 359–433 and x 441–515 for the same two words, heights agreeing at 24, and read it as a fixed minimum width being applied. `workbench/vaultview` already pins the measured 74 as `dialogButtonWDp` in its own dialog; nothing else does, and `patterns/modal` sizes no action, the footer's widgets being the caller's. Decide whether the measured push-button width is the modal's to state, so a dialog's footer is the platform's width everywhere instead of in one app.
...
719. **[decide]** **Nothing holds key focus when a dialog opens.** The reviewer: the platform's sheet shows the "Save As" field ringed, and mindchat's settings dialog draws no ring anywhere. `patterns/modal` traps Tab and owns a focus cycle but nominates no initial holder. Decide what a dialog focuses when it opens — the first field, the default action, or nothing — and whether that is the pattern's to say or the caller's.

720. **[decide]** **Two selection blues stand on screen at once.** The reviewer measured the dialog's list row at `(0,100,225)`, the platform's emphasized content selection, and the window's own sidebar row behind it at `(19,111,201)`, the same solid value in both appearances and so not an alpha over the rail. Two separately chosen blues for one job. Decide which is the selection, and whether a rail's differs from a list's by measurement or by accident.

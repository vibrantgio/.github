...
cuts phases from what is ruled, in the order ruled. A dated reading
stays as written, a record of what was measured that day; an item
that states contrast as a WCAG 2 ratio gets its Lc reading appended
when it comes up for ruling, never rewritten (ruled 2026-09-17).
...
## BO. From CE2.4c, the class layer and the mirror
...
425. **[bug]** **`CardFill` and `PushButtonFill` dark were measured over the tinted #232a2e plane,** so both carry the reference desktop's wallpaper cast against the untinted #1e1e1e window; four reviewers found it independently. Same class as SidebarMaterial's re-recording: re-measure both off the untinted captures (a grouped box and a push button in the dark appearance with tinting off), tokens, both tsv copies, ADR-019. Joined 2026-09-17 by `SidebarSearchFill` dark (`#2f3234`, b−r +5), read off the same tinted System Settings capture; all three re-measure off an untinted capture in the next permitted capture round. Folded 2026-09-17: the dark recess's step sign against the sidebar reads −3B in the tinted capture and +4B in ours, settled by the same untinted capture; the light sidebar material reads 247 over the desktop and 250 over other windows (the material shows what stands behind the window), so the recorded 247 stands as a known variance; the search field's clear mark has no capture behind its size or inset and joins this capture list. Also on the list: an unfocused text field carrying a value (the save dialog's only filled field is the focused one, so its inset is read through the focus ring). Joined 2026-09-18 by a field whose value reaches its trailing edge: no stored capture holds one, so CG4.21 mirrored the leading inset at the trailing end rather than reading it. Also: a pressed pop-up button (control-pressed-{light,dark}.png holds only the pressed Cancel push button). Joined 2026-09-18 by CG5.2's four: a push button under the pointer and NOT held (no capture holds one, so nothing measured a push button as exempt from the hover overlay and the exemption was deleted); an accent-filled button under the pointer (the overlay is laid on it unmeasured); a switched-off push button and a switched-off control that draws an edge (the save dialog holds neither, so the coverage read off its switched-off checkbox's fill carries the edge too); and one control captured under the pointer and then held in the dark appearance, which is what 544 turns on. Also (CG5.2): one dark control in both pointer states, to settle hover and press one 255th apart (hoverOverlay dark 24/255, pressOverlay 25/255); a switched-off text field, whose only fadeable part is its hairline; a sidebar row and a list row under the pointer, to settle the hover exemptions patterns/sidebar, mindchat and feeds assert in comments without a capture. Joined 2026-09-18 by CG5.2b's two: a control at the platform's SMALL size — a small pop-up beside a small push button and a small text field, which is what `CompactControlHeight`'s published 19 pt and the Compact line-box decision both still wait on; and a frontmost LIGHT window whose toolbar band carries the sidebar material under a bordered control, the one capture that would separate that control's `#ffffff` fill from the band it stands on in the light appearance, where every stored capture has it standing on the content's own white. Also: a checkbox or radio at the platform's small size (CompactCheckboxRowHeight is derived, 22 × 19/24) and an enabled checkbox beside a switched-off one. Joined 2026-09-18 by CG5.3b's one: a window whose TOOLBAR is drawn at the platform's small size, which `CompactToolbarControlHeight` carries the measured 36 until, and a frontmost light capture that resolves a toolbar control's drop shadow (560). Joined 2026-09-18 by CG5.3c's one: a switched-off CHECKED checkbox and a switched-off SELECTED radio — the save dialog's two switched-off boxes are both unset, so nothing measures what a switched-off control's own mark is drawn in and it takes the colour the switched-off label takes. Also (CG5.3d): an enabled unchecked checkbox beside a switched-off one (ours reads fainter, a 12-level edge on white against the solid faded fill) and a checked checkbox for the mark's stroke, terminals and inset. Also (CG5.3e): a fourth dark toolbar search field to settle the three fills read (Voice Memos #363636, Finder #262626, Mail #242d32) and the insets (13/7 against 10/8); a search field standing on a form. Also (CG5.3f): a toolbar band with room above a bordered control, to read the shadow's reach over the control. Also (CG5.3i): a chosen segment in a frontmost light window (the light on-state was read through an inactive window's fade). Also (CG5.3o): a search field reporting a match count (mail-find-* hold only the highlight). Joined 2026-09-18 by CG5.4's one, which is several readings in one frame: a POP-UP BUTTON WITH ITS MENU OPEN, the current item marked, in both appearances — no stored capture holds an open menu of any kind, so the menu's plane, its corner, its selection pill's inset and radius, its selected item's fill and text, the box its check is drawn in and the checkmark's own proportion are all named rather than read (`reference/macos/controls.md`, "Also open: the open menu"), and the same capture settles what a trigger draws while its menu stands, which CG5.4 drew as the held state. The open-menu capture (CG5.4) settles the plane against the window, the highlight's inset and corner, the row's padding, the gutter, the check's proportion, the hairline in light, and the shadow's offset; 644–649 folded here. The open pop-up capture is taken with the pointer on a row, both schemes: the menu's highlight is then its own measured value (the Accent entry), SidebarSelection standing in until then; 650 folded here. Also (CG5.5): a focused push button and a focused checkbox, to settle whether the halo straddles the control's outline (the Save dialog's field) or lies wholly outside it (Voice Memos' toolbar recess), and how much of it survives on an accent fill; 651 and 652 folded here. Also (CG5.6): a labelled toolbar control, for its side inset (ChromeMarkSideDp 7 reads symbol-only controls). Also (CG5.7): an enabled light history chevron, and a toolbar title or glyph on a light band that is not white (toolbarLabel's light value was read on white alone). Also (CG5.7): Safari's reload symbol for `refresh`, and a checked checkbox for `check`'s mark; both drawn to the grid until then. Also (CG5.10): Safari's reload at a size where the head must clear the ring (681 with 670); small symbols at sizes no capture holds, for the band's fixed device width. Narrowed 2026-09-18 by CG5.11, which read the band at three sizes and left three holes: a symbol standing between 12.33 px and 16 px of covered extent, and one over 19.31, so the set steps its band at 20 dp rather than running a line and draws the 1.4-unit band above 24 dp unmeasured; a BARE DIAGONAL band standing alone in a toolbar band, for the diagonal allowance, which the one stored clear control does not give (its cross is a knockout inside a solid disc, extent set by the disc) — this joins 671's checked checkbox on the same list; and the sidebar folder and the sidebar toggle BELOW 24 dp, the first for the flap's gap (it narrows to 0.41 px at 16 dp and the flap reads as one band with the body's top face) and the second for the second band's proportion at a smaller size. Also (CG5.11): the sidebar toggle's second band below 24 dp (its proportion to the first is unmeasured there). Also (CG5.12): a search field with a typed value in a second application (Mail or Finder), to settle the clear mark's trailing clearance (Voice Memos' 14 beside its own 13 inset where the platform's inset is 10) and its cap (square and round fit the one capture alike). Also (CG5.13): a bordered control standing after a bare window title, for the gap on that side (Finder's 14 is read on the title's leading side). Also (CG5.14): the platform's own OPEN panel, both appearances — no stored capture holds one, so what a chooser opens at has no reading and the vault-switch dialog states 560 × 340 dp instead (704). Also (CG5.14): the platform's open panel as a sheet on a window, both schemes, for its size, corner, position and shadow. Also (CG5.14): a floating surface's own shadow (a sheet or popover over a window; floatingShadow was fitted off a sidebar's edge shadow in finder-sidebar-shadow.png and reminders-sidebar-shadow.png, symmetric, and keeps that fit until then). Also (CG5.15): the platform's open panel captured as a sheet over one of our own windows, which CG5.15 can now present, to replace CG5.14's stated 560 by 340 with a measured size, corner, position and shadow. Also (CG5.16): a DEFAULT push button held down, and one under the pointer — `control-pressed-{light,dark}.png` differenced against the resting sheet changes x 359-432, y 501-524 and nothing else, which is the "Cancel" button, and the "Save" beside it keeps its `#157efb`, so both overlays are laid on the measured `defaultButtonFill` unmeasured. Also (CG5.18): the platform with Increase Contrast on, both schemes, so the reference holds what the semantic names become under the setting. Also (CG5.17): a dark sheet on an untinted desktop with its controls, to settle the neutral sheet under tinted controls (715 folded here with the tinted-plane readings). Also (711): the save dialog's dark sheet #232a2f is the tinted plane; re-measure it untinted, the dialog keeping WindowBackground meanwhile. Also (CG5.19): a genuinely bare toolbar control under the pointer (the hover overlay was read off a bordered one). Also (CG5.21): the open panel with its list focused, for the list's halo and selected row to the byte. Also (CG5.23): a light sidebar, window frontmost, list not holding the keyboard, wallpaper tinting off, to pair the unfocused pill's grey with SidebarMaterial's untinted reading and read its label on the pill the library paints (737); and every measured fill read as a flat value over a tinted capture re-read for a coverage through the material's dither (733: CardFill, PushButtonFill, SidebarSearchFill, ToolbarControlFill, ToolbarSearchFill, AlternatingContentBackground). Also (736): a list inside a platform inspector, to read its selection states (Notes' and Finder's inspectors carry properties, not lists). Also (CG7.1): a sidebar section with its disclosure, open and closed (Mail's mailbox groups; every stored Mail, Notes and Reminders sidebar is redacted), for the disclosure's box, inset, colour and the columns its two states land on (757, 766). Also (CG7.2): the dark sheet with its controls untinted (768/769: the sheet's fill and the toolbar greys were read on a tinted capture); a segmented control carrying a word (777). Joined 2026-09-23 by CG7.5's three: a BORDERED +/- PAIR UNDER A LIST (System Settings' Users & Groups or Network), for the pair's control height, its segment width and its mark's proportion — no stored capture holds one, and the pair is drawn at the toolbar control's measured 36 px meanwhile; a bordered control standing in a sheet's or a pane's BODY rather than in a toolbar band, which is what 788 turns on (System Settings' Appearance pane holds pop-ups, switches, a radio and three picture choosers and no segmented control at all, in both appearances, so it settles nothing); and a sheet whose body FILLS it, for what a form's last row leaves above the footer's hairline — the save panel gives its accessory region whatever height the sheet has left, so its own sixty-nine clear rows there are that panel's slack and not a reading. Also (CG7.4): a focused field with a typed value under the stored accent, to name the insertion point (Voice Memos' reads #178bfb, no name) and its halo together; a mirror textfield fixture holding a typed value follows that reading.
...
(`reviews/cg1.1-vaultview-fresh-eyes.md`).

429. **[decide]** **An inspector's row pitch.** Vaultview's outline and backlinks stand at the platform's list row, 20 dp, while the rail beside them stands at the sidebar's own 32; CG1.1 gave both the same inset pill and left the pitch where it was. The Language says an inspector is chrome and that a list in it selects as the sidebar does, and says nothing about its rows' height. One answer: an inspector's list is a sidebar's list at 32, or a content list at 20 wearing the sidebar's pill.

430. **[task]** **No stored capture holds a macOS inspector.** The nearest is Finder's info pane in `finder-window-light.png`, which is a pane inside the content and not a column beside it, so the inspector's fill, its seam and its rows are the Language's reading of what an inspector is rather than pixels. One capture closes it: a window showing its inspector, both appearances, window-bounded at 1x.

432. **[decide]** **A chrome foot drawing at the platform's small control height.** Vaultview's Rescan and Switch Vault now draw as small push buttons where the rest of the window draws at the reader's density. The Language sets density once per window and lets a component that must be smaller state its offset, as the chip does; whether a chrome foot may state one, and whether the platform in fact stands these controls small, wants a capture and a ruling.

433. **[bug]** **The backlinks pane cuts its first and last row through the glyphs.** `window-cited-{light,dark}` shows four citation rows whose text is halved at both ends of the pane, so the pane reads as broken rather than as scrolled. It predates CG1.1 and is untouched by it: the pane's height is a whole number of rows but its scroll region seats them half a row off.

434. **[decide]** **The checkbox and the radio still fill with the platform's text background** where the text field now fills with the surface it stands on. The Save dialog's two checkboxes are switched off, so no stored capture settles a box control standing on a fill that is not white; one that does would settle whether `control.Fill` follows `control.FieldFill`.

435. **[decide]** **The inspector's trailing lane.** Vaultview's outline and backlinks begin one lane — the sidebar pattern's measured 10 — inside the column and stop one lane plus the scrollbar's reserved gutter, 31, short of its trailing edge, so the selection pill is not inset alike on both sides the way the rail's is; a fresh reader called the difference "a visible dead margin down the right that nothing explains". The gutter is real and the note column reserves the same one, so closing it means the inspector's bar floats over its rows where the note's does not — two scrollbar conventions in one window. One answer for both columns.

436. **[decide]** **A window's two vertical boundaries read at different strengths.** The floating sidebar's edge measured #919191 light because the platform's separator lay on the window's plane there; the flush inspector's measured #e6e6e6 because it lies on the content. Both are the Seam entry applied correctly and the two did not look like one window. CG1.1's second pass took the plane away, which brings them to #dfdfdf and #e6e6e6 — near enough to read as one window and not the same number. Item 437 is the measurement that now decides both.

440. **[bug]** **Vaultview's three columns begin and end at three different heights.** The rail's toggle sits at y 18–33 and the vault's name at y 19–30, but the inspector's first element, the "Outline" heading, starts at y=47 — a 28 px hole in the top-right corner with nothing in it. At the foot the rail has a rule and two controls, the content has "29 lines" floating on bare white with no rule or band above it, and the inspector simply runs out of rows with forty pixels of empty chrome below. Nothing along either edge relates to anything else.

441. **[bug]** **The note's scroll indicator spans its whole track.** A note a few rows taller than its viewport draws a thumb of very nearly the track's own length — y 198–744 in `window-light.png` — which tells the reader neither where they are nor how much there is. A least proportion below which no indicator is drawn, or a least difference between the thumb and its track, closes it.

442. **[bug]** **The light status line is one tier paler than the rest of the window's secondary text.** "29 lines" measures `#969696` where the rail's placeholder and the outline's rows measure `#7c7c7c`, the platform's secondary label over the chrome. Dark has both at `#9a9a9a` and is right, so only the light value drifts.

443. **[decide]** **The platform's push button on a surface lighter than its own fill.** `PushButtonFill` is `#ececec` light, measured off a Save panel whose sheet is `#ffffff`; on the rail's `#f7f7f7` foot it is four levels DARKER than what it stands on, inside a `Separator`-over-fill hairline eleven levels off its own fill, and reads as a recessed patch rather than a raised control. Either the platform lifts a button over a chrome material and no capture holds one, or a chrome foot does not carry push buttons. Joins 432, which asks whether that foot may draw them small.

444. **[bug]** **The platform's row seam inside a grouped box is half the strength of the name it is drawn with.** MEASURED, `system-settings-grouped-box-{light,dark}.png`: the one-pixel line between two rows inside a box is `#ebebeb` on a `#f7f7f7` fill light and `#343a3e` on `#2a3034` dark — 12 levels off the fill and 10. `Separator` over `CardFill`, which is the name the Language gives that line, comes to 24 and 21. Either a box's internal row seam is a lighter line than a region seam, or `Separator` is two names.

445. **[decide]** **What marks a chosen colour, everywhere in the checkout.** CG2.1 measured the Appearance pane's own answer — a 3-point ring of the accent round the thumbnail with 1 point of the box's fill between them, and nothing about the thumbnail changing — and the themer's swatch row now draws it. The gallery's colour board, the picker's rows and every other place a row IS a colour still fill the row with the emphasized selection, which puts a second large flat colour beside the ones being compared. One answer for all of them.

448. **[bug]** **A third answer to what the platform draws between a sidebar and the content.** MEASURED, `system-settings-grouped-box-{light,dark}.png` at rows 240 and 540: the sidebar's fill ends at x=220, one pixel of `#ffffff` light and `#44444b` dark stands at x=222, and the content's plane then RAMPS from `#f3f3f3` up to its own `#ffffff` over about thirty points — the sidebar casting a shadow across the content. Item 437 has Finder drawing no line at all light and an unnamed `#434343` dark; this window draws a light line and a shadow. The ruling 437 asks for now has three measurements to settle, not two.

449. **[decide]** **One group per box, or related rows under one heading.** The themer draws five boxes of one row each, one heading apiece, where the Appearance pane puts "Colour" and "Text highlight colour" in a single box under "Theme". A fresh reader called the sectioning "fragmented". Which rows belong in one box is a question about the window's own content and not about how a box is drawn, and it is the shape CG2.1's task text named. Whether a titled group may hold two unrelated rows is the rule that would settle it for every window.

450. **[bug]** **`ControlAccent` and `SelectedContentBackground` are not what this machine paints.** A fresh reader measured the desktop's accent at `#157efb` and its sidebar selection at `#1068de`, against our `#007aff` and `#0064e1`; the Appearance pane's own selected-thumbnail ring corroborates the first — `#157efb` exactly, `system-settings-grouped-box-light.png` at x 455–457. `#0064e1` was described as "the pressed shade of system blue, used at rest". Both are published constants rather than values read off the platform, and every application in the checkout wears them.

451. **[bug]** **The dark window plane is neutral where the platform's is blue-grey.** `WindowBackground` dark is `#1e1e1e`, five to eight levels darker than the `#232a2e` the Appearance pane measures, while `CardFill` `#2a3034` matches that capture exactly. So our boxes stand off the plane by 12/18/22 where the platform's stand off by 7/6/6, and a blue-grey box sits on a neutral window: "in dark the boxes look pasted on". One surface was read off a capture and the other was not.

452. **[decide]** **A scrollbar thumb with no track.** In `list.Occupy` the bar takes a gutter and draws a resting thumb in it with nothing behind it, so on a short list the thumb reads as a stray mark rather than as a position in a longer one. Whether the platform's overlay bar ever shows a track, and at what length, is unmeasured — the stored reference holds `textedit-scrollbar.png` and its two neighbours, which are a document's bar and not a list's.

453. **[decide]** **Body copy in a markdown document is the most prominent text on the page.** `markdown.FromTokens` sets running text in the platform's `Text`, which is pure `#000000` light — darker than any heading in a window whose headings take `Label` at 85%. Read cold in the themer, "explanatory sample copy is the most prominent text on screen". Whether a document's body is `Text` or `Label` is one ruling for every reader of a markdown document.

454. **[decide]** **A syntax base whose own background all but coincides with the page it stands on.** In the themer's dark sample, catppuccin-mocha's `#1e1e2e` block sits on the previewed set's `#1e1e1e` plane — sixteen levels in one channel — so the fence is told from the page by its one-pixel border alone, in the one window whose job is showing what that base looks like. The fence wears the base's own background by design (ADR-019's 2026-08-19 addendum); what is unruled is what the page under a previewed fence should be when the two nearly meet.

455. **[decide]** **The platform's own accent colour is offered in the same shape as a colour out of a picture.** The themer's swatch row leads with the desktop's accent, drawn as one more card among the picture's, told apart only by the word "macOS" in its caption — and it is not a picture colour: it goes on changing under a window that has chosen it. Its own row in the theme colour group is the shape that would say so, and wants the height a group costs.

## BW. From CG3.1, sitedocs' tab strip

Filed 2026-09-17 from the task's own reading of every tab and its fresh-eyes
review (`reviews/cg3.1-sitedocs-fresh-eyes.md`).

456. **[decide]** **The Components tab holds three kinds of thing, and one of them is not a component.** Its sections run buttons, chips, fields and pickers (operated), badges, alerts, toasts and tooltips (read), then Paragraph, Icon and Layout — and rows, columns, spacers and insets are Gio's layout rather than a component at all. Control and signal are both components, so the cell's word is honest for all but the last; splitting it would mean sitedocs hand-picking sections instead of taking the catalogue's published groups, so the cut, if it is made, belongs in `components/gallery/inventory`. Declined in CG3.1 for that reason.

457. **[decide]** **The Patterns tab holds application chrome and a marketing page's sections under one word.** Navbar, sidebar, pane, table, modal, popover, tabs, accordion, card and group are what an application window is built of; hero, feature grid, pricing and testimonial are what a landing page is built of, and a reader after one scrolls through the other. Both are patterns, so the cell names one kind in the Language's own word, and the split would again be a cut sitedocs makes by section name rather than a group the catalogue publishes. Declined in CG3.1.

458. **[decide]** **The Markdown tab holds a reading sample and a syntax specimen.** Two sections: a whole document — headings, links, chips, lists, tasks, a table, a quote and a fence — and a fenced code block drawn in the chosen syntax base. Both are one renderer's output on one kind of source, and a "Code" cell would name something this window does not hold, there being no code view, only a fence inside a document. Declined in CG3.1; what would change the answer is the syntax base becoming a thing a reader chooses here rather than in the themer.

459. **[decide]** **Sitedocs' strip does not partition the window, and says nothing about its rule of division.** Docs is prose about the subjects of the four tabs after it — its outline lists "Colour: the platform's set", "Typography", "Markdown documents", "Icons" — so a reader after typography has two destinations and nothing to choose between them; Components and Patterns cannot be told apart from two bare nouns by anyone outside the Language; and Docs is the one tab with no header band to explain itself. Read cold: "the labels do not partition". One answer is a strip that says prose on one side and specimens on the other; another is a Docs tab that states its own scope where the other five do.

460. **[decide]** **"Markdown" names a file format where its neighbours name subjects of the design system.** The tab holds how running prose renders under the theme, which is what its own band says; the cell points at a syntax. Read cold as "the only tab whose word misdirects", with the suggestion it sit beside Typography rather than at the far end. The word is the catalogue's group name, so moving it moves every consumer of `components/gallery/inventory`.

461. **[decide]** **A tab holding one section opens on a band that repeats the cell above it.** Since CG3.1 the Typography tab is the type scale alone, so its section heading reads "Typography" two rows under the strip cell reading Typography, and the phrase that adds anything — "every typography role, its name and size beside a line set in it" — sits a thousand pixels away at the right margin in the muted foreground. The group tabs already drop the group banner for this reason; a section heading is not that banner, and the label here is the inventory's own section title, which this window is guarded against rewording. The same stranding is on the Colours tab's caption, and it has nowhere to go at a narrower window.

462. **[decide]** **The workbench switches a window's top-level sections with a control the platform does not use.** `patterns/tabs` draws an unenclosed row of left-aligned text labels under a two-pixel underline; macOS switches the top-level sections of a reference window with a source list or with a segmented control in the toolbar. Read cold in sitedocs, where a source list already stands inside the Docs tab, so the platform's own idiom is nested under the foreign one — and where the strip spends a full band across 1200 px to carry 480 px of labels, with the rest empty where the platform would put a toolbar. Every window standing on `patterns/tabs` moves with the answer.

463. **[decide]** **Six labels in one strip, in four grammatical shapes.** Docs, Colours, Typography, Components, Patterns, Markdown: three plurals, one mass noun, one format name, and one abbreviation — Docs is the only word shortened, where Colours and Components were not. Whether a strip's cells are held to one shape, and whether this one's word is Docs or Guide, is a copy ruling for every strip in the workbench.

## BX. From CG3.2, the gallery's captions
...
475. **[decide]** **The themer has no window chrome where the platform's own choices window has three regions.** Read cold beside System Settings' Appearance pane: "no titlebar, no toolbar band, no separator, no sidebar, no bottom bar … this reads as a settings *page* in a frame — the shape of a web preferences screen, not of an app window." The window controls are the platform's and absent from a harness render, so that half is a misread; what stands is that the page is a margin and a stack of boxes, where the platform frames the same job in a toolbar and a sidebar. Whether a one-pane choices window owes the platform a toolbar is the ruling.

476. **[decide]** **The themer marks a chosen thing four different ways.** A ring round the picture's swatch, a filled pill for the code face, a full-width filled row in the style list, a filled capsule in the appearance switch. The platform's own pane uses one idiom — a ring and an emboldened caption — for every chooser on it and reserves the filled row for value lists. Each of the four was argued for on its own (a filled swatch card put a second flat colour beside the colours being compared; a list row is marked the way a list marks one), and no ruling has ever compared the four.

477. **[decide]** **The picture row's captions carry two kinds of fact in one slot, and the first cell carries none.** "#007AFF · macOS" is a provenance and "#829F5E · 31%" is a share, in the same position and the same type; the dropped picture's own cell has an empty caption slot beside five that are filled, so the caption baseline has a hole in it. The shares also read out of order (38, 31, 2, 19, 11) because the ranking is by vividness, which the group's title row then spends a sentence explaining.
...
System Settings' sidebar, and from the measurement itself. The review
read the height, the corner, both fills, the absence of a stroke and
the placeholder's colour back as the platform's; everything below is
what it found around them, none of it in CG4.8's result.

494. **[decide]** **The search field's prompt is set about a fifth larger than the platform's.** Cap height 12 px against the sidebar field's 10, x-height 9 against 7 — BodyLarge's 16 sp where the platform sets its field at ~13 pt. `controls.md` already records the same gap against the push button's label cap band and leaves it, because typography is not the density scale's to move. It is one role's size, and it moves every field and every label set in that role with it. Joined 2026-09-18: the picker's label reads a fifth larger than the platform's in the same bezel (CG5.1's fresh eyes), and BodyLarge declares 0.5 px tracking that the library's typeset has never spent while the design bundle's browser did (CG5.6 makes the bundle mirror the library); whether the tracking token is ever spent is Rene's. Also (CG5.3b): a toolbar's title is set regular where the platform sets it semibold. Also (CG7.1): feeds' rail labels and section headings read a size under the platform's and lighter.
...
## CF. From CG5.1, the picker's form trigger as the platform's pop-up

Filed 2026-09-18 from the measurement itself and from the fresh-eyes review
of the gallery's picker row beside the save dialog's pop-up and the Finder
toolbar's pop-up and pull-down. The review confirmed every number the task
set, off the pixels and independently; everything below is what stands
around them. Two of its findings were already filed and are confirmed
rather than refiled: the focus ring drawn inside the control at half the
platform's width is pool 405, now with the platform's own 4 px ring outside
the bezel measured and both ring colours read; the dark fill's blue cast
against a neutral window is pool 425, a fourth independent finding.

The one item this section filed, 540 — a switched-off control fading its
...
the gallery's picker row beside the Finder toolbar's two controls and the save
dialog's pop-up. The review confirmed every number the task set — the pop-up
pair at 8 by 11 in both variants, nine clear columns from the trailing edge,
one colour for the mark and the label, the capsule at r = 12 on h = 24, and the
dark toolbar fill of 38 on a 28 band against the platform's 38 on 30 — and read
the form trigger's corner back as the platform's continuous curve within
0.15 px down the arc. One of its findings was this task's own and was fixed
rather than filed: the light rim the platform does not draw. Three of the rest
were already filed and are confirmed rather than refiled: hover and press one
255th apart in the dark appearance is the reading `controls.md` records and
425's list carries as 544; the dark fill's wallpaper cast is pool 425, now a
...
## CH. From CG5.3c, the switched-off checkbox as one fill

Filed 2026-09-18 from the measurement itself. No fresh eyes were named for
this task; the fill and the absence of an edge were read back off the
regenerated gallery goldens by the worker and are in the commit body.

568. **[bug]** **A component's op order moves text pixels across the whole gallery page.** Removing one `paint.FillShape` from the checkbox moved 66 scattered text pixels in `components-light.png` and 85 in `components-dark.png` by one 255th, in families the change does not touch, because the glyph atlas packs differently when the op stream changes. The regenerated images are stable across runs, so the gate still works, but every component change now carries unrelated text churn in its golden diff and a real one-255th text regression would be invisible inside it. Confirmed again by CG5.9 (675): the gallery's components goldens moved 453/691 one-255th text pixels on an unrelated op-stream change. Also (728): a static render holds no focus, so no golden can show a dialog's opening state; the live focus tests are the record.

## CL. From CG5.3n, the sidebar as the platform's inset panel

Filed 2026-09-18 from the measurement itself and from the fresh-eyes review of
vaultview's whole window in both schemes beside the owner's Voice Memos
capture and the dark Finder one. The task's own result — an inset rounded
panel at a margin of 8 with a 1 px rim, an 18 dp corner and a measured
shadow — is recorded in `controls.md` under "What the sidebar panel measures".

622. **[decide]** **One rectangle cannot carry the panel's shadow on all four sides.** The library spreads a shadow as a linear ramp from one peak at the edge of a rectangle sunk below the shape, which is the shape `toolbarControlShadow` is fitted to. Fitted to the panel's content side, its leading margin and the margin above it, black at 13/255 over 24 px sunk 9 lands 9,096 sampled pixels at an rms of 1.19 of 255 — and misses the 8 px of plane BELOW the panel by 8 to 11 of 255. Fitted to all four sides at equal weight it lands every side within about 3 of 255 but halves the ramp the reader actually sees across the content. The platform's shadow is blurred and lit from above; what would close this is a two-lobe shadow — an ambient ring and a sunk key — which nothing in the library draws. Joined 2026-09-18: the toolbar control's shadow plateaus where the platform's ramps from its first row (CG5.3o's fresh eyes), the same one-rectangle model read on a capsule.
623. **[decide]** **The dark sidebar material is a gradient across the panel, not a flat fill.** MEASURED, `finder-window-untinted-dark.png`: the panel's interior runs `#1b1b1b` at its leading rim to `#212121` at its trailing rim, six of 255 across 300 px, at every sampled row; `notes-window.png` and `reminders-window.png` agree in kind. `SidebarMaterial` carries the flat `#1c1c1c`, which is the mode and not the drawing. Whether a chrome fill may be a gradient at all is the decision; nothing else in the set is one.
624. **[note]** **The platform's panel corner is a continuous curve and Gio draws a circle.** A circle fitted to the rim's centreline at three corners lands at r=17.35 with an rms of 0.12 to 0.15 of a pixel, but the platform's arc leaves the straight edge about 13 px from the corner where a circle of that radius leaves it at about 11. `clip.RRect` has circular corners only, so every rounded thing in the library carries this; the pill's own corner already records the same finding at 8.
626. **[decide]** **Two measured gaps now stand between two chrome marks side by side.** The band leaves 14 between two bordered controls (`notes-toolbar.png`, carried as mindchat's `controlGapDp`) and the panel leaves 18 between two bare marks (`voicememos-multi-folder-2026-09-18.png`, carried as `pane.MarkGapDp`). Both are measured and they are not the same number, so a window with both placements spends two. Whether the panel's marks should take the band's spacing, or the two are genuinely different runs of chrome, is unruled.
...
task; the band's clipped edge was read off the new list reading's own capture
and the columns are in the commit body. The published mechanism the task named
— a viewport handed down the context and clipped with the deferred macro —
was built, measured against `workbench/sitedocs`' goldens, found to cut halos
that stand nowhere near a viewport edge, and replaced; 673 records why.
...
## CP. From CG5.8, feeds' rail beside the platform's two sidebars

Filed 2026-09-22 from the fresh-eyes review of feeds' whole window in both
appearances beside `voicememos-multi-folder-2026-09-18.png` and
`finder-window-untinted-dark.png`. The task's own result — the row's three
columns, the symbol's measured colour and the count placed by its drawn
pixels — the reviewer read back independently and found faithful to the byte;
everything below is what stands between the rows, which this task did not
touch. The reply is in `reviews/cg5.8-feeds-fresh-eyes.md`.

693. **[note]** **The document mark is a portrait figure in a column of square ones.** MEASURED by the fresh eyes off feeds' rail: our document draws 14 across by 20 down in the row's 24 dp square. The platform's row symbols measure 16–20 by 16–19 — Finder's Desktop 20×16, Applications 16×17, Documents 14×19, Voice Memos' folder 20×16. The mark is at the bottom of the width range and the top of the height range, which is what makes a column of them read taller than the platform's. Also (CG7.1, 765): the document mark's stroke never reaches its colour at the row's size, the weight half of this item.
...
## CR. From CG5.16, the default button's fill

Filed 2026-09-22 from the work itself. No fresh eyes were named; the fill and
the dialog's boundary were read back off the regenerated goldens against the
captures and the numbers are in the commit bodies.

710. **[note]** **The folder browser's names now begin 48 dp in from the row, not on the screen's own column.** CG5.16 put the folder mark at the sidebar row's measured `SymbolInset` 17 with the name at `LabelInset` 48, which is the only reading the reference holds for a symbol beside a name. In the vault-switch dialog that is exactly the platform's pair from the surface's own inset. On the full-screen picker the row also spends its own 8 dp of air first, so a name stands 56 in where the "Choose a vault" heading and the breadcrumb lead at 24. Nothing measured says a content browser's row uses the sidebar's columns; it was the only measured column available. Worth the owner's eye beside the platform's own open panel.
...
## CX. From CG7.1, feeds' rail as a pane

Filed 2026-09-23 from the fresh-eyes review recorded in
`reviews/cg7.1-feeds-fresh-eyes.md` and from the work itself. One reviewer was
handed feeds' whole window at 1x in both appearances, one group collapsed,
beside `mail-window.png` and `voicememos-multi-folder-2026-09-18.png`, the
recorded misreads, and one question: does this window's sidebar read as the
...
`save-dialog-{light,dark}.png` and
`system-settings-grouped-box-{light,dark}.png`, and from the fresh-eyes review
recorded in `reviews/cg7.5-mindchat-fresh-eyes.md`. One reviewer was handed
mindchat's whole window with the dialog open at 1x in both appearances, beside
the two save-dialog captures, the recorded misreads, and one question: does
this dialog read as a macOS sheet, and what is wrong with it.

787. **[decide]** **The platform's sheet parts its footer from its body with a hairline and stands its buttons 21 rows under it; `patterns/modal` draws neither.** MEASURED 2026-09-23 (`reference/macos/controls.md`, "What the sheet's own hairlines measure"): a 1 px `separatorColor` line runs edge to edge of the save sheet at y=479 — `#e6e6e6` light and `#393f43` dark, the name flattened onto the sheet to the byte on every channel — and the 65 px band under it carries the 24 px buttons centred, 21 clear rows above and the sheet's own 20 px inset below. The pattern's footer draws no line and leaves its own gap above the buttons (13 rows in mindchat's dialog); its comment now carries the reading and says why it does not spend it. Drawing the line and the 21 moves every dialog in the organization at once, which is why it is a decision and not one window's body.
788. **[decide]** **Two bordered controls now stand in a sheet's BODY at the toolbar band's measured 36 dp.** The templates' segmented control (CG7.2) and the well's `+`/`−` pair (CG7.5) both take `ToolbarControlHeight`, because the toolbar's is the only bordered control the reference measures — System Settings' Appearance pane holds pop-ups, switches, a radio and three picture choosers and NO segmented control at all, in both appearances, so it settles nothing. Every bordered control the save sheet itself measures is 24: both push buttons and both pop-ups. The fresh-eyes reviewer made the same reading from the window alone — "within this sheet's own control family, field 28, popup 24, buttons 24, list row 28, the segmented control is 35 and the +/− control is 36". Decide whether a bordered control standing in a body takes the sheet's 24 or the band's 36; the capture that would settle it is on 425.
789. **[note]** **The settings dialog's height is now the sum of its rows.** `SettingsBodyHeight` is arithmetic over the row constants rather than a stated 300, so the dialog measures 560 × 340 where it measured 560 × 412, and adding a row to the form changes the dialog's size. Nothing pins the dialog's own size. A form that has to stand in a sheet of a fixed height needs somewhere to spend the slack that is not a void above its last row, and no capture says where the platform puts it: the save panel's own sixty-nine clear rows above its footer hairline are that panel's accessory region absorbing the slack.
...
791. **[decide]** **A bordered control standing inside a window casts a drop shadow and the reviewer read it as the clearest un-macOS signal in the frame.** `controls.md` measures that shadow on a TOOLBAR BAND, and `components/button`'s `ChromeShadow` draws it wherever a chrome control is drawn; mindchat's templates control spends it on the dialog's own white because in the light appearance its fill is that same `#ffffff` and nothing else tells it from the sheet. CG7.5 took it off the `+`/`−` pair, which stands on a fill of its own. Decide whether the shadow belongs to the control or to the band it stands in — and if the control keeps it, what a light control standing on white without it is told from.
792. **[decide]** **The dialog carries four type sizes where the platform's sheet carries one.** Measured cap bands: the segment labels at 8 rows, the row labels, the checkbox label and the caption at 9, the push-button labels at 10, the field values at 11, the title at 12 — against a platform sheet whose labels, values, pop-up contents and button labels all measure 10. The damaging one is inside a single row: the label is two steps smaller than the value it names, and the two are centred on the control rather than sharing a baseline (the platform puts "Save As:" and "Untitled" on one). This is the Typography roles, not this dialog: `BodyMedium` for a label beside `BodyLarge` in a field.
793. **[note]** **The web-search row's checkbox is the Material drawing.** It is `icons.ToggleCheckBox` / `ToggleCheckBoxOutlineBlank` from the Material set, rasterised through ivg: the reviewer measured a 14 px box with a 2 px hard outline and a corner of about 1, against the platform's measured 16 px square at a corner of 5 with no edge of its own (`controls.md`, the save sheet's two "Options:" boxes). `components/input` already draws the measured checkbox; this row does not use it.
794. **[note]** **A field's placeholder takes `secondaryLabelColor`, the same grey its own row label takes.** The reviewer could not tell a placeholder from a value in the Base URL row for that reason and read the platform's placeholder as tertiary, "distinctly lighter". CG7.4 landed the placeholder at `SecondaryLabel` when it moved the value to `labelColor`; no stored capture holds a platform field with a placeholder in it, so the name was chosen and not read.
795. **[decide]** **The templates' segmented control stands across the whole form where every other row begins on the field column.** It leads at the label column's own leading edge, which lines up with the longest label and with nothing else, so the form reads as two grids stacked. CG7.2 put it there because four provider names do not fit the field column at this dialog's width — the field column is 257 dp and "OpenRouter" alone wants most of a quarter of it. Decide whether a control too wide for the field column may leave it, or whether the form's width answers for it instead.
796. **[decide]** **The providers well still has no bottom bar, no rim and no seam under its caption.** CG7.5 made the `+`/`−` the platform's bordered pair, which is the half of this the plan named; what the platform puts under a source list is a bordered bar holding that pair, and the well has a plain panel with the pair standing loose on its last row. The remainder of 772.
...
798. **[note]** **The push button's corner is 5 where the sheet's measures 4.** Read off the flat top run of the default button: the platform's fill starts three columns in on its first row and is solid by the third, ours four in and solid by the fourth. One pixel, recorded because nothing else in the reference holds a push button's corner.

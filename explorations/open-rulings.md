...
768. **[decide]** **The dark dialog's own fill is neutral `#1e1e1e` while every control standing on it was measured on the platform's tinted `#232a2f` sheet.** The reviewer measured the consequences on both sides: our sheet separates from the window behind it by less than half the platform's step (ΔY 0.0050 against 0.0119) while the controls on it separate from the sheet by roughly twice the platform's (the field's edge 0.0189 against 0.0097, "Cancel" 3.15:1 against 1.84:1). The chrome values themselves are the platform's to the byte — `#2c3338`, `#333a3f`, `#2a3034` — so what is wrong is the surface under them, not them. The light appearance has the mirror of it: the providers well reads 1.08:1 on white and 2.20:1 on the dark fill, so one box is a faint recess in one appearance and a raised plate in the other. Decide what a floating surface's dark fill is, since `windowBackground` is evidently not what the platform's sheet carries.
769. **[note]** **The segmented control's greys and its label are the TOOLBAR's names, strictly neutral, beside a sheet whose other controls carry the platform's blue-tinted greys.** `ToolbarControlFill`, `ToolbarControlSeam` and `ToolbarLabel` were all read off Finder's untinted windows; the sheet's own greys were read off a tinted save dialog. The reviewer read the four greys at 38/58/64/73 against the tinted values everywhere else in the same frame, and the control's labels at `#4d4d4d` light and `#e9e9e9` dark where the sheet's other primary text is `#242424` and `#e0e1e2`. It is the same root as 768 seen from the control's side.
...
771. **[note]** **The providers well's selected row is a square-cornered bar.** Measured at x 258–395, y 262–289 with a corner of 0 to 2 px, where the platform's list selection is a rounded rect at r ≈ 5–6. It is `SelectedContentBackground` painted at radius 0, which is what `components/list`'s content selection draws everywhere.
...
773. **[note]** **A text field draws its own value at pure black and pure white where the platform draws `labelColor`.** The reviewer read "OpenAI" in the Name field peaking at `(0,0,0)` light and `(255,255,255)` dark, against `(36,36,36)` and `(224,225,226)` — the platform's own values — for the pop-up's value, the push button's label, the checkbox's label and the list's rows in the same frame. It is `components/input`'s field text and it is every field in the organization, not this dialog's.
774. **[note]** **The modal's drop shadow is a symmetric halo with no vertical offset.** Measured on all four sides of the dialog at about 22 to 23 px of reach and a 7.4% peak multiply, identical above and below. The platform's window shadow is offset downward and much larger; the stored captures exclude it, so this was read off ours alone and there is nothing measured to set the offset from.
...
777. **[note]** **No stored capture holds a segmented control carrying a word, so a worded segment's own inset is not measured.** `controls.md` records that not one control in the five stored toolbar bands carries a word; `components/button` therefore sizes a worded segment at its label plus the clearance the SYMBOL path spends at either side, which is the nearest measured number it holds. What IS measured is that one control's segments are equal — Finder's four view segments are 37 px each — so the widest segment sets the width of all of them and a stated width divides evenly. A capture of a worded segmented control would settle the inset.

...
814. **[note]** **A body segment's WIDTH is still the toolbar's number.** The mark box now follows the platform — the control's height less the seven rows above and six below the pop-up's mark shows, which is eleven in a 24 dp control — but the clearance at each side is `control.ChromeMarkSideDp`'s 7, read off symbol-only controls in five toolbar BANDS. A body segment therefore measures 25 px on no capture at all, and the `+`/`−` pair narrowed from 77 to 51 because of it. The capture that settles it is already on 425's list (a bordered `+`/`−` pair under a list); this records that the height, the shape and the mark's room are now measured and the width alone is not.
...
816. **[note]** **The two places spend different vertical room around a mark.** A band control centres a 24 dp mark box in 36 px, six rows clear above and below; a body control now leaves seven above and six below. Both are measured — the band's 24 is a symbol's own covered extent in five toolbar bands, the body's 7 and 6 are where the Save dialog's pop-up stands its mark — but they are readings of different things, and nothing says what the platform would do with a symbol in a 24 px control, which no stored capture holds.

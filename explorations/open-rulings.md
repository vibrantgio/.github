...
757. **[decide]** **No stored capture holds a sidebar section's disclosure, so its box, its inset and the line it stands on are derived and not measured.** `mail-window.png` and `mail-window-light.png` are redacted down the sidebar — the dark one reads `#232a2e` on 288,025 of 294,272 sampled pixels with no glyph anywhere — and `notes-window.png` and `reminders-window.png` are redacted the same way; Finder's three headings and Voice Memos' "My Folders" carry no control at rest. CG7.1 recorded the gap in `controls.md` and derived the three numbers from readings the reference does hold. What would close it: a sidebar whose sections collapse, the pointer resting on one heading so the control is showing, and a second frame with that section collapsed, window-bounded at 1x with wallpaper tinting off.
...
760. **[note]** **`patterns/shell`'s `SidebarHeaderMain` cannot compose a pane, and every window with one now composes its own frame.** The layout lays its sidebar as a flush Flex column and pins its navbar slot to `NavbarHeight(d)`, which is the density's bar height; a pane needs the platform's measured 52 dp band and a shadow painted after the columns. vaultview, mindchat and now feeds each write the same twenty lines. Whether the shell grows a pane layout or the three frames stay app-local is unruled.

761. **[note]** **`patterns/shell.NavbarHeight`'s doc states 52 dp Comfortable and 40 dp Compact and the function returns 28 and 19.** `ControlHeight` is 24 at the comfortable density and `PaddingY` 2, so the sum is 28; the doc was written against a taller control height and never followed it down. Nothing reads the prose, but it is what sent CG7.1 looking for a 52 dp band in a window that had 28.

762. **[note]** **feeds' whole-window golden is taken through the live layers, where every other golden in the package is taken through a static render path with the pinned shaper.** The window has no static path — its layers are the only composition of it that exists — so the frames shape with the theme's own shaper. Every run in the window is Latin and the embedded Roboto leads the collection, so none of them reaches the platform's fonts; a window that grew a rune Roboto does not carry would need the static path first.

763. **[note]** **feeds' rail does not scroll.** The column lays its sections out from the panel's strip down and stops at the panel's foot, so a tree with more groups and feeds than the panel is tall simply loses the rest. The window's fixture has three groups and six feeds, which is why nothing shows it.

764. **[note]** **The rail's row labels and section headings are a size under the platform's and lighter on the page.** MEASURED by the fresh eyes off feeds' rail: our row label's cap is 9 px against Voice Memos' 10, at the same x-height and the same `#262626`, and lays down 0.177 of ink per advance-by-cap against the platform's 0.262; the heading reads 1.32 per advance px against "My Folders"' 1.61 at an identical cap height and colour. Part of the ink gap is the platform's own stem darkening, which is not ours to reproduce; the cap-height shortfall is. CG7.1 was told not to move typography, so both stand.

765. **[note]** **The document mark's stroke never reaches its colour, which is the weight half of 693.** MEASURED by the fresh eyes: our mark's dominant value is `#5a` over a `#b6` halo where Voice Memos' folder plateaus at `#00`, and it carries 64.1 of ink against the platform's 113.1 — 43 per cent less — in a figure that is already 14 across by 20 down against the platform's 20 by 16. 693 files the shape; this is the same mark reading mid-grey beside a `#262626` label where the platform's reads black.

766. **[note]** **A section's disclosure lands on two trailing columns, one per state, because the turn is about its box's centre.** The set's chevron covers 8 of its 24-unit box closed and 11.7 turned, so a box placed to land the closed mark on the rail's measured 17 puts the open one at 14. CG7.1 chose the centres as the invariant — the fresh eyes confirmed both states centre on one column and only the edges disagree — and the alternative is two boxes, which is two drawings.

767. **[note]** **A continuous corner measured as the distance from the corner to where the arc meets the straight edge reads 11 where a circular fit to the same pill reads 8.** The fresh eyes read Voice Memos' selection pill that way and called ours, at the recorded `SelectionRadius` of 8, the tighter corner. `controls.md` already records the same effect on the PANEL's corner — the arc leaves the straight edge ~13 px out where a circle of that radius leaves it at ~11 — so the two readings are of the same curve by two rulers. What is unruled is whether this library draws circular corners at the measured radius or grows a continuous corner of its own.

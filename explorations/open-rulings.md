...
461. **[decide]** **A tab holding one section opens on a band that repeats the cell above it.** Since CG3.1 the Typography tab is the type scale alone, so its section heading reads "Typography" two rows under the strip cell reading Typography, and the phrase that adds anything — "every role a surface reads in" — sits a thousand pixels away at the right margin in the muted foreground. The group tabs already drop the group banner for this reason; a section heading is not that banner, and the label here is the inventory's own section title, which this window is guarded against rewording. The same stranding is on the Colours tab's caption, and it has nowhere to go at a narrower window.

...
555. **[bug]** **A switched-off checkbox and radio keep their own fill where the platform fades it.** `components/input` fades the switched-off box's edge and leaves its interior at `control.Fill` — the platform's text background — in every appearance. The reference measures the switched-off box's fill at the enabled fill taken to 170 of 255 over the surface it stands on (`controls.md`, "a switched-off control's fill": `#f2f2f2` light on the sheet's white, `#2e3439` dark on its `#232a2f`), which is what `control.Faded` already computes for the button. The box is the one control in the library whose interior does not fade at all.
556. **[decide]** **The platform's switched-off checkbox draws no edge, and ours draws one.** `controls.md` reads the save dialog's two switched-off "Options:" checkboxes as a fill with no edge column — "its rim is a one-pixel antialiased ramp from the fill to the sheet", the way the pop-up above it meets the sheet. CG5.2 ruled that a switched-off control keeps its whole bezel on the ground that no capture holds a switched-off control that draws an edge; the checkbox is a switched-off control and it holds no edge, so either the checkbox is the exception the ruling did not see or the ruling narrows. One reading of one capture, which is why it is a decision and not a bug.
557. **[bug]** **The checkbox's and radio's focus ring is the one part of them still painted outside the box they report.** The ring rides 2 to 4 dp clear of the 16 dp glyph, so in the measured 22 dp row it paints at −1 and at 22 of a footprint running 0 to 21 — one row and one column outside on every side, in both densities (read off `components/input`'s regenerated `checkbox-light-focused.png` and `radio-light-focused.png`: 23 painted rows in a 22 dp footprint). CG5.5 is the task that settles what a focus halo is; recorded here so it is settled with a number rather than rediscovered.
...
559. **[decide]** **`CompactCheckboxRowHeight` is derived, and one capture would measure it.** 22 × 19/24 = 17.4, rounded to 17 — the same derivation `CompactFieldHeight` carries. The capture that closes it is the small-control capture already on 425's list; a checkbox drawn at the platform's small size, in both appearances, window-bounded at 1x, would settle this number with it.

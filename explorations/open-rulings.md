...
## CN. From CG5.5, the one focus halo

Filed 2026-09-18 from the measurement itself and from the fresh-eyes review of
the gallery's focus sheet in both schemes beside the Save dialog's focused
"Save As:" field. The task's own result — a four-px band in
`keyboardFocusIndicatorColor`, straddling the control's box two px out and two
px over, drawn in one place every control shares and deferred past the clip a
`widget.Clickable` puts around what it wraps — is recorded in `controls.md`
under "What the dialog captures measure". The review confirmed the colour and
the width off the rendered sheet independently.

651. **[decide]** **The reference's two focused controls do not place the halo the same way.** The Save dialog's focused "Save As:" field straddles its own box: its band covers x 262–265 against a box running x 264–495, so two px lie past the box and two over it, and the field's own `#f3f3f3` edge column reads through the band at x=264. Voice Memos' focused toolbar search recess puts the whole band outside: `voicememos-multi-folder-search-2026-09-18.png` gives x 696–699 leading, y 43–46 above and y 81–84 below, with the recess's own `#e8e8e8` fill unblended at x=700. Both are four px wide and both are read column by column. A bezel whose drawn fill is inset inside its frame would reconcile them, and nothing in either capture measures a frame apart from a fill. The library draws the dialog field's placement, which is the reading CG5.5 was given.
652. **[decide]** **Half the halo disappears on an accent-filled control, and swallows a checkbox's bezel.** `keyboardFocusIndicator` is the accent at 127/255, so the half of the band lying over a Filled button's own accent composites to `#0071fa` against that fill's `#007aff` — nine of 255 — and the visible indicator is two px where every other control on the sheet shows four. Read by the fresh eyes off `focus-{light,dark}.png`; dark is worse, the band reading as an antialiasing artefact around the fill. The same two px over a checkbox's one-px edge leave that edge as a six-of-255 shade, so a focused unchecked box and a focused text field are one construction at two sizes. Whether the platform places the band differently on a control that fills itself cannot be read: no stored capture holds a focused push button, a focused checkbox or a focused radio, and the capture that would close it is on `controls.md`'s open list.
653. **[bug]** **The pop-up trigger's chevron pair draws nine rows against the measured eleven.** MEASURED, `save-dialog-{light,dark}.png` and recorded by CG5.1: the pair is 8 px wide and 11 tall, five rows for each chevron with one clear row between, at a stroke of about 1.5 px perpendicular. `components/gallery/testdata/golden/focus-light.png` draws it 8 by 9 — four rows each, the same one-row gap, and a thinner arm — which is why the fresh eyes read the two chevrons as merging into one blob at 1x.
654. **[note]** **A second macOS pop-up button carries its chevrons on an accent badge.** The fresh eyes described the platform's pop-up as two separated white chevrons on an accent-blue rounded badge at the trailing edge, and read the library's grey-on-grey pair as switched off beside it. The reference holds one pop-up — the Save dialog's "File Format", whose mark is `labelColor` on the control's own `#ececec` with no badge anywhere — and that is what CG5.1 drew. Two drawings of one control, one of them unmeasured.
655. **[note]** **The focus sheet's light "card" and "chrome" rows are the same test.** `CardFill` and `SidebarMaterial` are both `#f7f7f7` in the light appearance, so the sheet's three surfaces are two there; dark they differ, `#2a3034` against `#1c1c1c`. The claim the sheet carries — that the halo's colour follows what it stands on rather than being chosen per control — is proved on two light fills and three dark ones.
656. **[bug]** **`workbench/sitedocs`' colours-tab goldens were stale before CG5.5 touched them.** Both appearances differed by 46 pixels, all of them the scrollbar thumb's lower end at x 1172–1177, y 381–389 — the tab's content is two px taller than the stored image was recorded at. Verified pre-existing by stashing `components` and re-running: the same 46 pixels, deterministically. CG5.5 regenerated them with its own, and what made the tab taller between the recording and now is unfound.
657. **[decide]** **A deferred halo leaves the scroll area it was drawn in.** `op.Defer` resets the clip, which is what lets a focused control keep the box it reports and still paint a band past it; it also lets that band paint past a scroll viewport, a pane's clip and a column boundary. The same is already true of a bordered toolbar control's drop shadow, and the platform does draw a focus ring outside the control's frame. No stored capture holds a focused control scrolled to the edge of a list, so what the platform does when the band would leave the viewport is unmeasured.

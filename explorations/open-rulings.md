...
## CO. From CG5.11, the band as a device width

Filed 2026-09-18 from the measurement itself. No fresh eyes were named for
this task: the band was read off the platform's own captures, the rule was
drawn from those readings, and the result was read back off the regenerated
icon sheet in both schemes, all in the commit body. The capture holes the
reading left are on 425's list.

683. **[decide]** **The library's clear control is a bare cross where the platform's is a filled disc.** MEASURED at 1x off `voicememos-multi-folder-search-2026-09-18.png`, x 996-1009: the platform draws the control that empties a field as a filled disc 13.06 px across with the field's own fill showing through a cross knocked out of it, the cross covering 5.31 by 5.32 px with arms 1.05 px across perpendicular. `components/icons`' `clear` is a bare cross drawn to the set's 20-unit diagonal allowance, which is a different drawing and not a smaller one. Whether the library's clear control should be the platform's disc is a question about what the control IS, and it is unruled; CG5.11 recorded the numbers and drew nothing.
684. **[bug]** **The search field draws its magnifier a pixel under the platform's own.** `components/input`'s `markDp` is 16, derived when a round form took 20 of the grid's 24 units: "the set draws a mark to 20 of its 24 grid units, so the square that comes out at the platform's size is 12.9 × 24/20 = 15.5". CG5.10 measured the round allowance at 13 units and CG5.7 the drawing's own extent at 16.195, so the glyph now covers 10.80 px at 16 dp plus the band's widening — 11.19 against the capture's own 12.33 (`mail-window.png` x 878-890, `voicememos-window.png` x 657-669). The square the field draws the mark in was never re-derived after the allowance moved.
685. **[note]** **The second band takes the first band's widening, and its proportion below 24 dp is not measured.** The sidebar toggle's list lines are 0.93 px against a 1.39 band in the one capture that holds them, which is 0.669. The widening is an offset of every edge the drawing lays down, so at 16 dp the second band comes out 1.01 px against the first's 1.32 — 0.76 — and at 20 dp the same. Exempting it would put it at 0.47 instead, further from the capture, so the uniform offset is what CG5.11 drew; which is right is unmeasured, and the capture that would settle it is on 425's list.

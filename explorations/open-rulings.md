...
608. **[bug]** **The bordered toolbar control's dark rim is five levels short of the measured pixel.** `controls.md` records that control's edge as `#404040` and states that `separatorColor` over the fill gives `#3b3b3b`, "so neither name lands it and the rim is recorded as the pixel". `components/internal/control`'s `ToolbarRim` flattens `separatorColor` anyway and draws `#3b3b3b`; the toolbar search recess beside it has a measured pixel token of its own (`tokens.PlatformColors.ToolbarSearchRim`, `#4d4d4d`) and the bordered control has none. Three controls in vaultview's band now wear the short rim, and so does every chrome mark elsewhere.
609. **[decide]** **The band re-flows when the find opens, and a control lands on a column edge.** The trailing cluster is right-aligned, so turning the 38 dp search capsule into a 220 dp recess with its count beside it slides the two actions before it 228 dp leading; the vault switch then straddles the boundary between the note column and the inspector, 8 dp of its capsule over the content's fill and 29 over the inspector's. Finder collapses and expands its own search capsule the same way, so the travel is the platform's, but Finder has three controls before its search where this window has two and no column edge falls under any of them. Whether the recess should stand at all times, as Mail's and Voice Memos' do, is unruled.
610. **[bug]** **A band control's drop shadow is covered over one column and not over the other.** The shadow reaches 23 dp past a control 36 tall in a band 52 deep, so up to 18 rows fall below the band. Over the inspector they are drawn — that column's fill is painted before the row — and over the note column they are not, because the column repaints its own fill after the row. One shadow, two answers, decided by which column happens to be underneath. The platform casts below the band and does not cut it off (`finder-window-light.png` reads 244 under a control at y=44 against 255 in the empty band, recovering to 251 by y=54), so what is wrong is the inconsistency and not the reach.
611. **[decide]** **Nothing in the band is grouped.** The platform runs related toolbar items into ONE capsule with hairline dividers — Mail's reply/reply-all/forward is a single 110 px capsule, its archive/delete/junk another 111, Finder's share/tag/more one 110 — and spends a tight gap inside a cluster (8 in Mail) against a wide one between clusters (28). Every control this library draws in a band is a capsule of its own, so nothing can say that two actions belong together, and the one gap it spends is Finder's 16. A chrome segmented control is a component the library does not have.
612. **[decide]** **The window's navigation and the document's own name are not in the band.** Finder puts back and forward in the toolbar as one segmented capsule beside the window title; this window keeps them as bare chevrons in the note column below the band, and the band's title names the VAULT while the note actually open is named only by the breadcrumb under it. A macOS document window's toolbar says what is open and how to go back.
613. **[decide]** **The find's count stands beside the field rather than in it.** "2 of 3" is loose text in the band 10 dp leading of the recess, while the recess's own clear mark sits inside it at the trailing end. No stored capture holds a find bar with a count, so what the platform does with one is unmeasured.
614. **[bug]** **The find's match marks are painted over the scrollbar's thumb.** Three 3 px bands land inside a thumb spanning 120 px, in the same highlight fill the prose marks use, so the thumb reads as four disconnected pieces in light and as a thumb with holes punched through it in dark, where the marks are DARKER than the thumb they sit on. A match's place belongs in the track, behind or beside the thumb.
615. **[bug]** **The scrollbar's thumb is over-inked in both schemes and in opposite directions.** It draws an opaque 109 in light where the platform's overlay thumb over white lands nearer 166, and an opaque 159 in dark where the platform lands nearer 100. In the light note it is the second most prominent thing on screen.
616. **[decide]** **The vault's search and the note's find are drawn identically.** The rail carries a magnifier and the word "Search" in a field; the band carries a magnifier that opens find-in-note and, opened, a second field with the same prompt. Nothing tells a reader which one searches the whole vault.
617. **[bug]** **A pinned row and live scrolling content simply touch.** The breadcrumb row stays put while the document scrolls under it with no hairline, no inset and no fill of its own, so scrolled text passes 2 px beneath it; the inspector's outline is cut the same way, its rows clipped mid-glyph 15 px under the "Outline" header on identical fill.

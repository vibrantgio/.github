...
562. **[bug]** **A toolbar title is set at the wrong weight.** mindchat's chat title is TitleSmall regular where the platform sets a toolbar's title semibold, which fresh eyes called the most recognisable typographic tell of the band.
...
565. **[bug]** **The density walkers do not carry every density field.** `theme/export`'s JSON emits `controlHeight`, `chipHeight` and the two paddings only — `fieldHeight`, `rowHeight` and `checkboxRowHeight` never reach it — and its CSS omits `checkbox-row-height`. Two measured numbers are therefore invisible to a consumer reading the sheet or the parameters. CG5.3b added its own field to both walkers and left the three behind it as they were rather than widening its diff.
566. **[bug]** **The search field's chrome variant is one control for two places.** The platform draws a sidebar recess and a TOOLBAR recess at different heights — Voice Memos' toolbar field measures 36 px in both appearances against System Settings' sidebar field at 28, both already in `controls.md` — and `input.Chrome` draws the sidebar's height wherever it stands. A search field in a toolbar band is 8 dp short of the platform.

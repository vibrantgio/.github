...
- [ ] Create a throwaway module outside `.repos/` — placement is D1 and must not gate the technical question.
...
- [ ] `go.work` — add the new `use` line; it is generated from `find .repos -name go.mod`, so regenerate rather than hand-edit.
